#!/usr/bin/env python3
"""Convert the official BF16 Qwen3.5-122B-A10B checkpoint for Vulkan.
...
MAGIC_SHARED = b"OQ35SHR\0"
MAGIC_EXPERT = b"OQ35EXP\0"
...
MAIN_PARAMETERS = 122_111_526_912
ACTIVE_PARAMETERS = 9_771_288_576
CHECKPOINT_PARAMETERS = 125_086_497_008
CHECKPOINT_BYTES = 250_173_007_840
...
assert (GATE_SCALE, GATE_WEIGHT) == (0, 98_304)
assert (UP_SCALE, UP_WEIGHT) == (1_671_168, 1_769_472)
assert (DOWN_SCALE, DOWN_WEIGHT) == (3_342_336, 3_440_640)
assert EXPERT_RECORD_BYTES == 5_013_504
...
class SafeStore:
    ...
    def __init__(self, directory: Path):
        ...
        if int(index.get("metadata", {}).get("total_size", -1)) != CHECKPOINT_BYTES:
            raise ValueError("checkpoint index total_size does not match official Qwen3.5-122B-A10B")
        self.weight_map: Dict[str, str] = index["weight_map"]
        if len(self.weight_map) != 1949:
        ...
        if missing:
            raise FileNotFoundError(
                f"checkpoint incomplete: {len(missing)}/39 shards missing: {missing[:6]}"
            )
        ...
        if dtype_parameters != {"BF16": 125_086_490_096, "F32": 6912}:
            raise ValueError(f"unexpected checkpoint dtype totals: {dtype_parameters}")
...
def build_plans() -> List[Plan]:
        ...
        if layer in LINEAR_ATTENTION_LAYERS:
            ...
            result.extend([
                ...
                Plan(f"{runtime}.ab_proj",
                     ...
                     Q4G64T, layer, (128, DIM), "concat_rows"),
    ...
    if len(result) != 627 or len({plan.name for plan in result}) != len(result):
...
def validate_plan(store: SafeStore, plan: Plan) -> None:
    ...
    if plan.transform == "direct":
        ...
        if len(infos) != 1 or infos[0].shape != (12_288, 1, 4) or plan.shape != (12_288, 4):
...
def validate_existing_shared(path: Path) -> None:
    ...
    qwords = fields[-10:]
    if qwords[3] != 627 or qwords[5] != path.stat().st_size or \
...
def validate_config(metadata_dir: Path) -> None:
    ...
    if config.get("model_type") != "qwen3_5_moe":
        raise ValueError("metadata is not Qwen3.5 MoE")
...
def main() -> None:
    ...
    parser.add_argument("--source", type=Path, required=True,
                        help="directory containing all 39 official safetensors shards")

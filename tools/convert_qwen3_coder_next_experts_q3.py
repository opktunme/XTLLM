#!/usr/bin/env python3
"""Repack Qwen3-Coder-Next Q4 experts as compact signed Q3/K64.

The Q4 source is read-only. Interrupted partial outputs are retained, and an
incompatible partial causes a new numbered partial to be created instead of
truncating or deleting any existing file.
"""
...
import convert_qwen3_coder_next as c


b = c.b
Q4_RECORD_BYTES = c.EXPERT_RECORD_BYTES
Q3_RECORD_BYTES = 1_277_952
Q3_GATE_SCALE = 0
Q3_GATE_WEIGHT = 32_768
Q3_UP_SCALE = 425_984
Q3_UP_WEIGHT = 458_752
Q3_DOWN_SCALE = 851_968
Q3_DOWN_WEIGHT = 884_736
Q3_DATA_BYTES = Q3_RECORD_BYTES
RECORDS = c.LAYERS * c.EXPERTS


def q3_header(file_bytes: int) -> bytes:
    return b.EXPERT_HEADER.pack(
        c.MAGIC_EXPERT, 1, b.HEADER_BYTES, c.DIM, c.MOE_DIM,
        c.LAYERS, c.EXPERTS, 0, Q3_RECORD_BYTES,
...
def validate_header(path: Path, expected: int, q3: bool) -> None:
    with path.open("rb") as source:
        raw = source.read(b.EXPERT_HEADER.size)
    if len(raw) != b.EXPERT_HEADER.size:
        raise ValueError(f"short expert header: {path}")
    fields = b.EXPERT_HEADER.unpack(raw)
    wanted = b.EXPERT_HEADER.unpack(
        q3_header(expected) if q3 else c.expert_header(expected)
    )
    if fields != wanted:
        raise ValueError(f"incompatible expert header: {path}")
...
def repack_matrix(
    ...
    q3_scales = b.f32_bf16(
        b.bf16_f32(scale_u16) * np.float32(7.0 / 3.0)
    ).tobytes()
    ...
    for nibble in range(8):
        code4 = (
            (packed_q4 >> np.uint32(4 * nibble)) & np.uint32(15)
        ).astype(np.int16)
        signed4 = np.where(code4 >= 8, code4 - 16, code4)
        signed3 = np.rint(
            signed4.astype(np.float32) * np.float32(3.0 / 7.0)
        ).astype(np.int16)
        chunks |= (signed3 & 7).astype(np.uint32) << np.uint32(3 * nibble)
...
def write_record(output: BinaryIO, source: mmap.mmap, record: int) -> None:
    source_base = b.HEADER_BYTES + record * Q4_RECORD_BYTES
    target_start = output.tell()
    specs = (
        (c.GATE_SCALE, c.GATE_WEIGHT, c.MOE_DIM, c.DIM,
         Q3_GATE_SCALE, Q3_GATE_WEIGHT),
        (c.UP_SCALE, c.UP_WEIGHT, c.MOE_DIM, c.DIM,
         Q3_UP_SCALE, Q3_UP_WEIGHT),
        (c.DOWN_SCALE, c.DOWN_WEIGHT, c.DIM, c.MOE_DIM,
    ...
    if output.tell() - target_start != Q3_DATA_BYTES:
        raise AssertionError("Q3 record size drift")
...
def main() -> None:
    ...
    final = args.output
    expected_q4 = b.HEADER_BYTES + RECORDS * Q4_RECORD_BYTES
    expected_q3 = b.HEADER_BYTES + RECORDS * Q3_RECORD_BYTES
    validate_header(source_path, expected_q4, q3=False)
    if source_path.stat().st_size != expected_q4:
        raise ValueError("Q4 expert source has an unexpected size")
    if final.exists():
        validate_header(final, expected_q3, q3=True)
    ...
    if partial.exists():
        try:
            validate_header(partial, expected_q3, q3=True)
            data = partial.stat().st_size - b.HEADER_BYTES
        ...
        if remainder:
            ...
            print(
                f"resuming exact Q3 record boundary {completed}/{RECORDS}",
                flush=True,
            )
    ...
    with source_path.open("rb") as source_file, \
        ...
        output.seek(b.HEADER_BYTES + completed * Q3_RECORD_BYTES)
        for record in range(completed, RECORDS):
            write_record(output, source, record)
            if (record + 1) % c.EXPERTS == 0:
                output.flush()
                os.fsync(output.fileno())
                print(
                    f"Q3 experts layer {(record + 1) // c.EXPERTS}/{c.LAYERS}",
                    flush=True,
                )
        output.flush()
        os.fsync(output.fileno())

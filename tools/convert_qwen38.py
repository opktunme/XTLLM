#!/usr/bin/env python3
"""Convert the official Qwen3.8-Flash-Next FP8 checkpoint for Vulkan.
...
* ``ple-fp8.ovp`` contains the official FP8 n-gram lookup table in one
  contiguous, directly-indexed file.  The table stays on NVMe; only the 16
  selected 160-byte rows are decoded for each token.
...
Vision and MTP weights are deliberately not emitted.  This converter performs
no inference.
...
MODEL_REVISION = "970c569adaca6b35532111fd6b27351b2baefe50"

MAGIC_SHARED = b"OQ38SHR\0"
MAGIC_EXPERT = b"OQ38EXP\0"
MAGIC_PLE = b"OQ38PLE\0"
...
PLE_HEADER = struct.Struct("<8s8I8Q")
...
# Headline counts published for the architecture.  Exact converted tensor
# totals are printed from the checkpoint rather than used as a brittle gate.
MAIN_PARAMETERS = 125_000_000_000
ACTIVE_PARAMETERS = 6_000_000_000
CHECKPOINT_BYTES = 185_502_232_570

PLE_PARTS = 128
PLE_ROWS_PER_PART = 2_500_012
PLE_ROW_BYTES = 160
PLE_ROWS = PLE_PARTS * PLE_ROWS_PER_PART
PLE_HEADER_BYTES = 4096
...
EXPERT_DATA_BYTES = DOWN_WEIGHT + DIM * MOE_DIM // 2
EXPERT_RECORD_BYTES = 2_613_248
assert (GATE_SCALE, GATE_WEIGHT) == (0, 51_200)
assert (UP_SCALE, UP_WEIGHT) == (870_400, 921_600)
assert (DOWN_SCALE, DOWN_WEIGHT) == (1_740_800, 1_792_000)
assert EXPERT_DATA_BYTES == 2_611_200
assert EXPERT_RECORD_BYTES == 2_613_248
...
class SafeStore:
    ...
    def __init__(
        ...
        expected_tensors: int = 152_089,
        expected_shards: int = 131,
        minimum_parameters: int = MAIN_PARAMETERS,
        checkpoint_label: str = "Qwen3.8-Flash-Next-FP8",
    ):
        ...
        if int(index.get("metadata", {}).get("total_size", -1)) != expected_bytes:
            raise ValueError(f"checkpoint index total_size does not match official {checkpoint_label}")
        ...
        if len(by_shard) != expected_shards:
        ...
        if missing:
            raise FileNotFoundError(
                f"checkpoint incomplete: {len(missing)}/{expected_shards} shards missing: {missing[:6]}"
            )

        dtype_bytes = {"F32": 4, "BF16": 2, "F8_E4M3": 1, "I64": 8}
        ...
        dtype_parameters: Dict[str, int] = {name: 0 for name in dtype_bytes}
        for info in self.infos.values():
            dtype_parameters[info.dtype] += product(info.shape)
        if sum(dtype_parameters.values()) <= minimum_parameters:
    ...
    def array(self, name: str) -> np.ndarray:
        info = self.info(name)
        dtype = {"F32": "<f4", "BF16": "<u2", "F8_E4M3": "u1", "I64": "<i8"}[info.dtype]
        return np.ndarray(
            info.shape,
            dtype=dtype,
...
def _build_fp8_e4m3_lut() -> np.ndarray:
    ...
    for encoded in range(256):
        sign = -1.0 if encoded & 0x80 else 1.0
        ...
        result[encoded] = sign * value
    return result


FP8_E4M3_LUT = _build_fp8_e4m3_lut()


def fp8_f32(values: np.ndarray, inverse_scales: np.ndarray) -> np.ndarray:
    """Decode the checkpoint's [128,128] block-scaled E4M3 tensor."""
    raw = np.asarray(values, dtype=np.uint8)
    rows, columns = raw.shape
    decoded = FP8_E4M3_LUT[raw]
    scales = np.asarray(inverse_scales, dtype=np.float32)
    expected = ((rows + 127) // 128, (columns + 127) // 128)
    if scales.shape != expected:
        raise ValueError(f"FP8 scale shape {scales.shape} != {expected}")
    return decoded * np.repeat(np.repeat(scales, 128, axis=0), 128, axis=1)[:rows, :columns]
...
def build_plans() -> List[Plan]:
    result = [
        ...
        Plan("final_hc_norm", ("model.language_model.hyper_connection_mixer.hc_norm.weight",),
             F32, -1, (HC_DIM,)),
        Plan("final_hc_down", ("model.language_model.hyper_connection_mixer.input_mix_weight_down.weight",),
             Q4G64T, -1, (HC_LOWRANK, HC_DIM)),
        Plan("final_hc_up", ("model.language_model.hyper_connection_mixer.input_mix_weight_up.weight",),
             Q4G64T, -1, (HC_DIM, HC_LOWRANK)),
    ]
    for layer in range(LAYERS):
        source = f"model.language_model.layers.{layer}"
        runtime = f"layers.{layer}"
        for block in ("attn", "mlp"):
            hc = f"{source}.{block}_hyper_connection"
            result.extend([
                Plan(f"{runtime}.{block}_hc_norm", (f"{hc}.hc_norm.weight",),
                     F32, layer, (HC_DIM,)),
                Plan(f"{runtime}.{block}_hc_down", (f"{hc}.input_mix_weight_down.weight",),
                     Q4G64T, layer, (HC_LOWRANK, HC_DIM)),
                Plan(f"{runtime}.{block}_hc_up", (f"{hc}.input_mix_weight_up.weight",),
                     Q4G64T, layer, (HC_DIM, HC_LOWRANK)),
                Plan(f"{runtime}.{block}_hc_inject", (f"{hc}.block_inject_weight.weight",),
                     Q4G64T, layer, (HC_COUNT, HC_DIM)),
            ])
        if layer in LINEAR_ATTENTION_LAYERS:
            ...
            result.extend([
                ...
                Plan(f"{runtime}.ab_proj",
                     ...
                     Q4G64T, layer, (96, DIM), "concat_rows"),
        ...
        if layer == 1:
            ple = f"{source}.ple"
            result.extend([
                Plan(f"{runtime}.ple_key", (f"{ple}.key_proj.weight",), Q4G64T,
                     layer, (HC_DIM, DIM)),
                Plan(f"{runtime}.ple_value", (f"{ple}.value_proj.weight",), Q4G64T,
                     layer, (DIM, DIM)),
                Plan(f"{runtime}.ple_norm_key", (f"{ple}.norm_key.weight",), F32,
                     layer, (HC_DIM,)),
                Plan(f"{runtime}.ple_norm_query", (f"{ple}.norm_query.weight",), F32,
                     layer, (HC_DIM,)),
                Plan(f"{runtime}.ple_norm_conv", (f"{ple}.norm_conv.weight",), F32,
                     layer, (HC_DIM,)),
                Plan(f"{runtime}.ple_conv", (f"{ple}.conv1d.weight",), F32,
                     layer, (HC_DIM, 4), "ple_conv"),
            ])
    if len(result) != 923 or len({plan.name for plan in result}) != len(result):
...
def validate_plan(store: SafeStore, plan: Plan) -> None:
    ...
    if plan.transform == "direct":
        ...
        if len(infos) != 1 or infos[0].shape != (10240, 1, 4) or plan.shape != (10240, 4):
            raise ValueError(f"convolution shape mismatch for {plan.name}")
    elif plan.transform == "ple_conv":
        if len(infos) != 1 or infos[0].shape != (HC_DIM, 1, 4) or plan.shape != (HC_DIM, 4):
            raise ValueError(f"PLE convolution shape mismatch for {plan.name}")
    else:
...
def validate_checkpoint(store: SafeStore, plans: Sequence[Plan]) -> None:
    ...
    for layer in range(LAYERS):
        for expert in range(EXPERTS):
            ...
            expected = {
                f"{prefix}.gate_proj.weight": (MOE_DIM, DIM),
                f"{prefix}.up_proj.weight": (MOE_DIM, DIM),
                f"{prefix}.down_proj.weight": (DIM, MOE_DIM),
            }
            for name, shape in expected.items():
                info = store.info(name)
                scale = store.info(name + "_scale_inv")
                wanted_scale = ((shape[0] + 127) // 128, (shape[1] + 127) // 128)
                if info.dtype != "F8_E4M3" or info.shape != shape or \
                        scale.dtype != "BF16" or scale.shape != wanted_scale:
                    raise ValueError(
                        f"unexpected routed expert tensor {name}: "
                        f"{info.dtype} {info.shape} scale={scale.dtype} {scale.shape}"
                    )
    main_parameters = sum(
        product(info.shape)
    ...
    print(
        ...
        f"language+PLE indexed parameters={main_parameters:,}, "
        f"headline main~{MAIN_PARAMETERS:,}, active~{ACTIVE_PARAMETERS:,}, "
        f"tensors={len(store.infos):,}",
        flush=True,
    )
...
def fp8_tensor_chunks(
    ...
    values = store.array(name)
    scales = bf16_f32(store.array(name + "_scale_inv"))

    def iterate() -> Iterator[Tuple[int, np.ndarray]]:
        for first in range(0, rows, chunk_rows):
            source = values[first:first + chunk_rows]
            if source.shape[1:] != (columns,):
                raise ValueError(f"expert slice shape drift: {name}")
            block_scales = scales[first // 128:(first + len(source) + 127) // 128]
            yield first, fp8_f32(source, block_scales)
...
def write_f32(output: BinaryIO, store: SafeStore, plan: Plan) -> Result:
    ...
    elif plan.transform in ("conv", "ple_conv"):
        values = decode_array(store, plan.sources[0]).reshape(plan.shape)
        output.write(np.ascontiguousarray(values, dtype="<f4").tobytes())
    elif plan.transform == "concat_vector":
        for name in plan.sources:
...
def validate_existing_shared(path: Path) -> None:
    ...
    qwords = fields[-10:]
    if qwords[3] != 923 or qwords[5] != path.stat().st_size or \
...
def write_experts(
    ...
    with partial.open(mode, buffering=16 * 1024 * 1024) as output:
        ...
        for record in range(completed, records):
            ...
            start = output.tell()
            specifications = [
                (f"{prefix}.gate_proj.weight", MOE_DIM, DIM, GATE_WEIGHT, GATE_SCALE),
                (f"{prefix}.up_proj.weight", MOE_DIM, DIM, UP_WEIGHT, UP_SCALE),
                (f"{prefix}.down_proj.weight", DIM, MOE_DIM, DOWN_WEIGHT, DOWN_SCALE),
            ]
            for name, rows, columns, wanted_weight, wanted_scale in specifications:
                weight, weight_bytes, scale, scale_bytes = pack_q4g64(
                    output,
                    fp8_tensor_chunks(store, name, rows, columns),
                    rows,
                    columns,
                    cache_decoded=True,
                )
            ...
            if output.tell() - start != EXPERT_DATA_BYTES:
                raise AssertionError("expert record size drift")
            output.write(bytes(EXPERT_RECORD_BYTES - EXPERT_DATA_BYTES))
            done = record + 1
...
def write_ple(output_dir: Path, store: SafeStore) -> Path:
    """Write the exact official FP8 PLE table without materializing it in RAM."""
    final = output_dir / "ple-fp8.ovp"
    partial = output_dir / "ple-fp8.ovp.partial"
    data_bytes = PLE_ROWS * PLE_ROW_BYTES
    expected = PLE_HEADER_BYTES + data_bytes
    scale_name = (
        "model.language_model.layers.1.ple.ple_embedding."
        "ngram_embedding.weight_scale"
    )
    scale = float(bf16_f32(store.array(scale_name)).reshape(-1)[0])
    scale_bits = struct.unpack("<I", struct.pack("<f", scale))[0]
    if final.exists():
        if final.stat().st_size != expected:
            raise ValueError(f"bad existing PLE file size: {final}")
        with final.open("rb") as source:
            fields = PLE_HEADER.unpack(source.read(PLE_HEADER.size))
        if fields[:8] != (
            MAGIC_PLE, VERSION, PLE_HEADER_BYTES, PLE_PARTS,
            PLE_ROWS_PER_PART, PLE_ROW_BYTES, PLE_ROWS, scale_bits,
        ):
            raise ValueError(f"incompatible existing PLE file: {final}")
        print(f"PLE: keeping existing {final}")
        return final
    if partial.exists():
        raise ValueError(
            f"preserving interrupted PLE file {partial}; remove it manually only "
            "after confirming it is disposable"
        )

    header = PLE_HEADER.pack(
        MAGIC_PLE, VERSION, PLE_HEADER_BYTES, PLE_PARTS,
        PLE_ROWS_PER_PART, PLE_ROW_BYTES, PLE_ROWS, scale_bits, 0,
        PLE_HEADER_BYTES, data_bytes, expected, 0,
        0, 0, 0, 0,
    )
    prefix = "model.language_model.layers.1.ple.ple_embedding.ngram_embedding"
    with partial.open("xb", buffering=16 * 1024 * 1024) as output:
        output.write(header)
        output.write(bytes(PLE_HEADER_BYTES - len(header)))
        for part in range(PLE_PARTS):
            name = f"{prefix}.shard_{part}.weight"
            info = store.info(name)
            if info.dtype != "F8_E4M3" or info.shape != (PLE_ROWS_PER_PART, PLE_ROW_BYTES):
                raise ValueError(f"unexpected PLE shard {name}: {info.dtype} {info.shape}")
            values = store.array(name).reshape(-1)
            view = memoryview(values).cast("B")
            for first in range(0, len(view), 64 * 1024 * 1024):
                output.write(view[first:first + 64 * 1024 * 1024])
            output.flush()
            print(f"PLE part {part + 1}/{PLE_PARTS}", flush=True)
        if output.tell() != expected:
            raise AssertionError("PLE output size drift")
        os.fsync(output.fileno())
    partial.rename(final)
    print(f"PLE: {final} ({expected:,} bytes, scale={scale:.9g})")
    return final
...
def validate_config(metadata_dir: Path) -> None:
    ...
    if config.get("model_type") != "qwen4_exp":
        raise ValueError("metadata is not Qwen3.8-Flash-Next/Qwen4-Exp")
...
def main() -> None:
    ...
    parser.add_argument("--source", type=Path, required=True,
                        help="directory containing all 131 official FP8 safetensors shards")
    ...
    parser.add_argument("--skip-ple", action="store_true")
    ...
    if not arguments.skip_ple:
        write_ple(arguments.output, store)

...
public:
    QwenEngine(const Runtime& runtime, const SharedIndex& index,
        ...
        quasi_hidden_ = std::getenv("QWEN_QUASI_HIDDEN_PREFETCH") != nullptr;
...
private:
    ...
    void schedule_quasi_prefetch(uint32_t completed_layer) {
        if (!quasi_hidden_ || !decode_phase_ || completed_layer + 1 >= kLayers)
            return;
        expert_file_.cancel_batch(speculative_reads_);
        speculative_layer_ = completed_layer + 1u;
        const auto started = std::chrono::steady_clock::now();
        LayerSets& sets = layers_[speculative_layer_];
        const uint64_t signal = compute_.submit([&](VkCommandBuffer command) {
            Push push{1, kDim, float_bits(1e-6f), 0};
            kernels_.dispatch(command, kernels_.p().rms, sets.predict_norm,
                              &push, 1);
            compute_barrier(command);
            push = {kDim, 128, kDim / 4, kDim / 4};
            kernels_.dispatch(command, kernels_.p().quant,
                              sets.predict_quant, &push, kDim / 128);
            compute_barrier(command);
            push = {kExperts, kDim, kDim / 4, 0};
            kernels_.dispatch(command, kernels_.p().q8,
                              sets.predict_router_gemv, &push,
                              (kExperts + 7) / 8);
            compute_barrier(command);
            push = {kExperts, kTopK, 0, 0};
            kernels_.dispatch(command, kernels_.p().router,
                              sets.predict_router, &push, 1);
            compute_barrier(command);
        });
        compute_.wait(signal);
        invalidate_buffer(runtime_, predict_routing_);
        const uint32_t* routes =
            static_cast<const uint32_t*>(predict_routing_.mapped);
        std::vector<uint64_t> offsets;
        std::vector<void*> destinations;
        std::vector<uint32_t> ranks;
        speculative_count_ = 0;
        for (uint32_t rank = 0; rank < kTopK; ++rank) {
            const uint32_t expert = routes[rank];
            if (expert >= kExperts)
                throw std::runtime_error("Quasi-hidden router returned invalid expert");
            predicted_experts_[rank] = expert;
            if (device_cache_.contains(speculative_layer_, expert) ||
                host_cache_.contains(speculative_layer_, expert))
                continue;
            speculative_experts_[speculative_count_] = expert;
            offsets.push_back(expert_file_.offset(speculative_layer_, expert));
            destinations.push_back(
                speculative_staging_[speculative_count_].mapped);
            ranks.push_back(speculative_count_);
            ++speculative_count_;
        }
    ...
    void consume_quasi_prefetch(
        ...
        if (!quasi_hidden_ || !decode_phase_ || speculative_layer_ != layer)
            return;
        uint32_t selected_predictions = 0;
        for (uint32_t predicted : predicted_experts_)
            if (std::find(authoritative.begin(), authoritative.end(), predicted) !=
                authoritative.end())
                ++selected_predictions;
        prediction_total_ += kTopK;
        prediction_hits_ += selected_predictions;
        ...
        for (uint32_t expert : authoritative) {
            if (device_cache_.contains(layer, expert) ||
                ...
                continue;
            ++ssd_miss_total_;
            if (std::find(predicted_experts_.begin(), predicted_experts_.end(),
                          expert) != predicted_experts_.end())
                ++ssd_miss_predicted_;
        }
        ...
        speculative_useful_ += useful;
        speculative_wasted_ += speculative_count_ - useful;
        expert_file_.cancel_batch(speculative_reads_);
        speculative_count_ = 0;
        speculative_layer_ = UINT32_MAX;
    }
    ...
    std::array<uint32_t, kTopK> predicted_experts_{};

...
public:
    DeviceExpertCache(const Runtime& runtime, uint32_t slots)
        ...
        static constexpr std::array<uint32_t, kLayers> kPrefixProfile{
            13,26,32,32,31,25,33,22,32,27,30,27,29,18,29,31,21,21,33,28,
            31,29,25,31,33,31,31,32,31,35,25,33,31,33,25,25,23,33,30,36,
            26,34,37,26,22,34,29,35};
        static constexpr std::array<uint32_t, kLayers> kSuffixProfile{
            13,26,32,32,31,25,30,22,29,27,30,27,29,18,29,31,21,21,12,28,
            19,29,25,31,30,31,31,32,31,26,25,24,31,30,25,25,23,30,30,21,
            26,25,19,26,22,25,29,26};
        if (profiled_) {
            prefix_capacities_.assign(kPrefixProfile.begin(), kPrefixProfile.end());
            suffix_capacities_.assign(kSuffixProfile.begin(), kSuffixProfile.end());
        } else {
            prefix_capacities_.assign(kLayers, slots_ + 2u);
            suffix_capacities_.assign(kLayers, slots_ - 4u);
        }
        for (uint32_t layer = 0; layer < kLayers; ++layer) {
            const uint64_t partial_bytes =
                uint64_t(prefix_capacities_[layer]) * kDownScale +
                uint64_t(suffix_capacities_[layer]) *
                    (kExpertRecordBytes - kDownScale);
            if (partial_bytes > uint64_t(capacities_[layer]) *
                                    kExpertRecordBytes)
                throw std::runtime_error("Qwen partial profile exceeds VRAM budget");
        }
        layers_.resize(kLayers);
        for (uint32_t index = 0; index < kLayers; ++index) {
            Layer& layer = layers_[index];
            layer.prefix_arena = create_device_buffer(
                runtime, uint64_t(prefix_capacities_[index]) * kDownScale);
            layer.suffix_arena = create_device_buffer(runtime,
                uint64_t(suffix_capacities_[index]) *
                    (kExpertRecordBytes - kDownScale));
            layer.prefix_entries.resize(prefix_capacities_[index]);
            layer.suffix_entries.resize(suffix_capacities_[index]);
            device_bytes_ += layer.prefix_arena.allocation_size +
                             layer.suffix_arena.allocation_size;
        }
    }

    ~DeviceExpertCache() {
        for (Layer& layer : layers_) {
            destroy_buffer(runtime_, layer.suffix_arena);
            destroy_buffer(runtime_, layer.prefix_arena);
        }
    }

    struct Selection {
        std::array<uint32_t, kTopK> slots{};
        std::array<uint32_t, kTopK> suffix_slots{};
        std::array<bool, kTopK> misses{};
        std::array<bool, kTopK> prefix_misses{};
        std::array<bool, kTopK> suffix_misses{};
    };
    struct Batch2 {
    ...
    Selection resolve(uint32_t layer_index,
        ...
        for (uint32_t rank = 0; rank < kTopK; ++rank) {
            ...
            selected_bytes_ += kExpertRecordBytes;
        }
        resolve_part(layer.prefix_entries, layer.frequency, experts,
                     result.slots, result.prefix_misses,
                     prefix_hits_, prefix_misses_);
        resolve_part(layer.suffix_entries, layer.frequency, experts,
                     result.suffix_slots, result.suffix_misses,
                     suffix_hits_, suffix_misses_);
        for (uint32_t rank = 0; rank < kTopK; ++rank) {
            result.misses[rank] = result.prefix_misses[rank] ||
                                  result.suffix_misses[rank];
            if (result.misses[rank]) ++misses_;
            else ++hits_;
            if (!result.prefix_misses[rank]) resident_bytes_ += kDownScale;
            if (!result.suffix_misses[rank])
                resident_bytes_ += kExpertRecordBytes - kDownScale;
        }
        return result;
    }
    ...
    Batch2 resolve_two(
        ...
        (void)layer_index; (void)experts;
        throw std::runtime_error("verify2 is unavailable in partial-residency research");
    }

    DescriptorRange record(uint32_t layer, uint32_t slot) const {
        return prefix_record(layer, slot);
    }
    DescriptorRange prefix_record(uint32_t layer, uint32_t slot) const {
        return arena_range(layers_.at(layer).prefix_arena,
                           uint64_t(slot) * kDownScale, kDownScale);
    }
    DescriptorRange suffix_record(uint32_t layer, uint32_t slot) const {
        return arena_range(layers_.at(layer).suffix_arena,
            uint64_t(slot) * (kExpertRecordBytes - kDownScale),
            kExpertRecordBytes - kDownScale);
    }
    Buffer& arena(uint32_t layer) { return layers_.at(layer).prefix_arena; }
    Buffer& prefix_arena(uint32_t layer) { return layers_.at(layer).prefix_arena; }
    Buffer& suffix_arena(uint32_t layer) { return layers_.at(layer).suffix_arena; }
    uint32_t slots() const { return slots_; }
    uint32_t slots(uint32_t layer) const { return capacities_.at(layer); }
    uint32_t prefix_slots(uint32_t layer) const {
        return prefix_capacities_.at(layer);
    }
    uint32_t suffix_slots(uint32_t layer) const {
        return suffix_capacities_.at(layer);
    }
    ...
    uint64_t misses() const { return misses_; }
    uint64_t device_bytes() const { return device_bytes_; }
    uint64_t selected_bytes() const { return selected_bytes_; }
    uint64_t resident_bytes() const { return resident_bytes_; }
    void reset_metrics() {
        hits_ = misses_ = prefix_hits_ = prefix_misses_ = 0;
        suffix_hits_ = suffix_misses_ = selected_bytes_ = resident_bytes_ = 0;
    }

private:
    struct Entry { int32_t expert = -1; uint64_t age = 0; };
    struct Layer {
        Buffer prefix_arena{}, suffix_arena{};
        std::vector<Entry> prefix_entries, suffix_entries;
        std::array<uint32_t, kExperts> frequency{};
    };
    void resolve_part(std::vector<Entry>& entries,
                      const std::array<uint32_t, kExperts>& frequency,
                      const std::array<uint32_t, kTopK>& experts,
                      std::array<uint32_t, kTopK>& slots,
                      std::array<bool, kTopK>& misses,
                      uint64_t& hit_counter, uint64_t& miss_counter) {
        std::vector<bool> reserved(entries.size());
        for (uint32_t rank = 0; rank < kTopK; ++rank) {
            slots[rank] = UINT32_MAX;
            for (uint32_t slot = 0; slot < entries.size(); ++slot) {
                if (entries[slot].expert != int32_t(experts[rank])) continue;
                slots[rank] = slot;
                reserved[slot] = true;
                entries[slot].age = ++clock_;
                ++hit_counter;
                break;
            }
        }
        for (uint32_t rank = 0; rank < kTopK; ++rank) {
            if (slots[rank] != UINT32_MAX) continue;
            ++miss_counter;
            uint32_t victim = UINT32_MAX;
            for (uint32_t slot = 0; slot < entries.size(); ++slot) {
                if (reserved[slot]) continue;
                if (victim == UINT32_MAX || entries[slot].expert < 0 ||
                    (entries[victim].expert >= 0 &&
                     (frequency[entries[slot].expert] <
                          frequency[entries[victim].expert] ||
                      (frequency[entries[slot].expert] ==
                           frequency[entries[victim].expert] &&
                       entries[slot].age < entries[victim].age)))) {
                    victim = slot;
                    if (entries[slot].expert < 0) break;
                }
            }
            if (victim == UINT32_MAX)
                throw std::runtime_error("No partial expert-cache victim");
            entries[victim].expert = int32_t(experts[rank]);
            entries[victim].age = ++clock_;
            reserved[victim] = true;
            slots[rank] = victim;
            misses[rank] = true;
        }
    ...
    const Runtime& runtime_;
    uint32_t slots_;
    std::vector<uint32_t> capacities_, prefix_capacities_, suffix_capacities_;
    bool profiled_ = false;
    std::vector<Layer> layers_;
    uint64_t clock_ = 0, hits_ = 0, misses_ = 0, device_bytes_ = 0;
    uint64_t prefix_hits_ = 0, prefix_misses_ = 0;
    uint64_t suffix_hits_ = 0, suffix_misses_ = 0;
    uint64_t selected_bytes_ = 0, resident_bytes_ = 0;
};

struct Pipelines {
    VkPipeline embedding{}, rms{}, quant{}, q4{}, q4_residual{}, q8{};
    VkPipeline swiglu{}, router{}, expert_gate{}, expert_down{}, reduce{};
    VkPipeline expert_gate_batch{}, expert_down_batch{}, expert_down_suffix{};
...
public:
    Kernels(const Runtime& runtime, const std::filesystem::path& directory)
        ...
        pipelines_.expert_down_suffix =
            load("qwen35_expert_down_q4_suffix");
...
public:
    ...
    uint64_t selected_expert_bytes() const {
        return device_cache_.selected_bytes();
    }
    uint64_t resident_expert_bytes() const {
        return device_cache_.resident_bytes();
    }
...
private:
    ...
    void build_sets() {
        ...
        for (uint32_t layer = 0; layer < kLayers; ++layer) {
            ...
            sets.expert_gate.resize(device_cache_.prefix_slots(layer));
            sets.expert_down.resize(device_cache_.suffix_slots(layer));
            for (uint32_t slot = 0; slot < sets.expert_gate.size(); ++slot) {
                const DescriptorRange record =
                    device_cache_.prefix_record(layer, slot);
                sets.expert_gate[slot] = kernels_.set(
                    {whole(quant_), record, whole(routing_),
                     whole(expert_intermediate_)});
            }
            for (uint32_t slot = 0; slot < sets.expert_down.size(); ++slot) {
                const DescriptorRange record =
                    device_cache_.suffix_record(layer, slot);
    ...
    uint32_t run_token(uint32_t token, uint32_t position) {
        ...
        *static_cast<uint32_t*>(token_.mapped) = token;
        ...
        for (uint32_t layer = 0; layer < kLayers; ++layer) {
            ...
            selected_suffix_slots_ = selection.suffix_slots;
            ...
            if (tensor_split_ && progressive_experts_ && has_missing) {
                ...
                const auto submit_gate = [&](uint32_t rank, bool copy_part) {
                    uint64_t ready = 0;
                    if (copy_part) {
                        ...
                        ready = transfer_.submit([&](VkCommandBuffer command) {
                            const VkBufferCopy copy{0,
                                uint64_t(selected_slots_[rank]) * kDownScale,
                                kDownScale};
                            vkfn::CmdCopyBuffer(command, staging_[rank].handle,
                                device_cache_.prefix_arena(layer).handle, 1,
                                &copy);
                            dsv4::transfer_barrier(
                                command, device_cache_.prefix_arena(layer));
                        });
                        last_ready = ready;
                ...
                const auto submit_down = [&](uint32_t rank, bool copy_part) {
                    uint64_t ready = 0;
                    if (copy_part) {
                        ...
                        ready = transfer_.submit([&](VkCommandBuffer command) {
                            const VkBufferCopy copy{kDownScale,
                                uint64_t(selected_suffix_slots_[rank]) *
                                    (kExpertRecordBytes - kDownScale),
                                kExpertRecordBytes - kDownScale};
                            vkfn::CmdCopyBuffer(command, staging_[rank].handle,
                                device_cache_.suffix_arena(layer).handle, 1,
                                &copy);
                            dsv4::transfer_barrier(
                                command, device_cache_.suffix_arena(layer));
                        });
                        last_ready = ready;
                ...
                // A selected part that is already resident can start before
                // the other matrix region arrives.
                for (uint32_t rank = 0; rank < kTopK; ++rank) {
                    if (!selection.misses[rank]) continue;
                    if (!selection.prefix_misses[rank])
                        submit_gate(rank, false);
                    if (!selection.suffix_misses[rank] &&
                        gate_submitted[rank])
                        submit_down(rank, false);
                }
                ...
                for (uint32_t rank = 0; rank < kTopK; ++rank) {
                    ...
                    host_copy_bytes_ += kExpertRecordBytes;
                    if (selection.prefix_misses[rank])
                        submit_gate(rank, true);
                    if (selection.suffix_misses[rank])
                        submit_down(rank, true);
                    else if (!down_submitted[rank])
                        submit_down(rank, false);
                ...
                while (batch.reads.remaining) {
                    ...
                    if (prefix_ready[rank] && !gate_submitted[rank])
                        submit_gate(rank, selection.prefix_misses[rank]);
                    if (suffix_ready[rank] && gate_submitted[rank] &&
                        !down_submitted[rank])
                        submit_down(rank, selection.suffix_misses[rank]);
                }
                for (uint32_t rank = 0; rank < kTopK; ++rank) {
                    if (!selection.misses[rank] ||
                        !batch.sources.direct[rank]) continue;
                    if (!gate_submitted[rank])
                        submit_gate(rank, selection.prefix_misses[rank]);
                    if (!down_submitted[rank])
                        submit_down(rank, selection.suffix_misses[rank]);
    ...
    void record_experts(VkCommandBuffer command, uint32_t layer) {
        ...
        if (batch_experts_) {
            ...
            throw std::runtime_error(
                "BDA batching is unavailable in partial-residency research");
        } else {
            for (uint32_t rank = 0; rank < kTopK; ++rank) {
                push = {rank, kTopK * kMoeDim / 4, 0, 0};
                kernels_.dispatch(command, kernels_.p().expert_down_suffix,
                                  sets.expert_down[selected_suffix_slots_[rank]], &push,
                                  kDim / 8);
            }
        }
        compute_barrier(command);
    ...
    std::array<uint32_t, kTopK> selected_slots_{}, selected_suffix_slots_{};
...
int qwen35_cli_main(int argc, char** argv) {
    ...
    try {
        ...
        uint64_t selected_expert_bytes = 0, resident_expert_bytes = 0;
        ...
        uint32_t host_slots = 0;
        {
            ...
            selected_expert_bytes = engine.selected_expert_bytes();
        ...
        std::cout << "\ndecode throughput: "
                  ...
                  << "partial selected expert bytes resident: "
                  << (selected_expert_bytes ?
                        100.0 * double(resident_expert_bytes) /
                            double(selected_expert_bytes) : 0.0)
                  << "%\n"

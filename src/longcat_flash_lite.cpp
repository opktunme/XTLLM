...
constexpr uint32_t QL=1536, KVL=512, H=32, QHD=192, VD=128, CTX=256;
...
public:
    ...
    uint32_t run(uint32_t tok,uint32_t pos,std::vector<uint32_t>&history){if(pos>=CTX)throw std::runtime_error("LongCat short-context cap reached");*static_cast<uint32_t*>(token.mapped)=tok;flush_buffer(rt,token);history.push_back(tok);oefile.read_rows(history,oe_stage.mapped);dsv4::flush_buffer_range(rt,oe_stage,0,12*512);
...
private:
    ...
    void attention_block(uint32_t l,uint32_t s,uint32_t pos){auto&a=layers[l].attn[s];uint64_t sig=compute.submit([&](VkCommandBuffer c){Push p{1,D,float_bits(1e-5f),0};kern.dispatch(c,kern.p.rms,a.in_norm,&p,1);compute_barrier(c);p={D,128,D/4,D/4};kern.dispatch(c,kern.p.quant,a.hidden_q,&p,D/128);compute_barrier(c);p={QL,D,D/4,0};kern.dispatch(c,kern.p.q4,a.qa,&p,QL/8);p={KVL+64,D,D/4,0};kern.dispatch(c,kern.p.q4,a.kva,&p,(KVL+64)/8);compute_barrier(c);p={1,QL,float_bits(1e-6f),0};kern.dispatch(c,kern.p.rms,a.qa_norm,&p,1);p={1,KVL,float_bits(1e-6f),0};kern.dispatch(c,kern.p.rms,a.kva_norm,&p,1);compute_barrier(c);p={QL,128,QL/4,QL/4};kern.dispatch(c,kern.p.quant,a.qa_q,&p,QL/128);Push pk{KVL,128,KVL/4,KVL/4};kern.dispatch(c,kern.p.quant,a.kva_q,&pk,KVL/128);compute_barrier(c);p={H*QHD,QL,QL/4,0};kern.dispatch(c,kern.p.q4,a.qb,&p,H*QHD/8);p={H*256,KVL,KVL/4,0};kern.dispatch(c,kern.p.q4,a.kvb,&p,H*256/8);compute_barrier(c);p={l*2+s,pos,CTX,0};kern.dispatch(c,kern.p.store,a.store,&p,H);compute_barrier(c);kern.dispatch(c,kern.p.attention,a.attention,&p,H);compute_barrier(c);p={H*VD,128,H*VD/4,H*VD/4};kern.dispatch(c,kern.p.quant,a.ctx_q,&p,H*VD/128);compute_barrier(c);p={D,H*VD,H*VD/4,0};kern.dispatch(c,kern.p.q4r,a.out,&p,D/8);compute_barrier(c);});compute.wait(sig);}

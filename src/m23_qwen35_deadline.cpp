...
public:
    ...
    void begin_split(const std::vector<uint64_t>& offsets,
        ...
        batch.active = true;
        try {
            // Earliest-deadline-first: every gate/up prefix can unblock useful
            // expert work, while each down suffix is needed only after that
            // expert's SwiGLU.  Submit the complete prefix frontier first.
            for (uint32_t part = 0; part < 2; ++part) {
                for (uint32_t item = 0; item < offsets.size(); ++item) {
                    const uint32_t index = part *
                        static_cast<uint32_t>(offsets.size()) + item;

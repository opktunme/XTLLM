...
static void configure_deepseek(const Options& options, double ram_gib,
    ...
    if (ram_gib > 24.0) set_environment("DSV4_PLAIN_HOST_CACHE", "1");
    ...
    try {
        if (argc < 2) {
            std::cerr
                ...
                << "options: --ram-gib N --device-slots N --tokens N --think "
                   "--no-prewarm --plan\n";
            return 2;
        }
        ...
        if (!options.simulated_available_gib && ram_gib > host.available_gib - 2.0)
            throw std::runtime_error(

...
#include "m26_deepseek_v4_longctx.cpp"
#endif
...
struct Pipelines {
    ...
    VkPipeline qk{}, store_value{}, attention{}, attention_reduce{}, head_gate{};
    VkPipeline long_block_score{}, long_block_topk{};
    VkPipeline long_attention_selected{}, long_block_recall{};
...
public:
    Kernels(const Runtime& runtime, const std::filesystem::path& directory)
        ...
        pipelines_.attention = load(std::getenv("QWEN_LONG_ATTN_SHADER") ?
            std::getenv("QWEN_LONG_ATTN_SHADER") :
            "qwen35_long_attention_shared16");
        pipelines_.attention_reduce = load("qwen35_long_attention_reduce");
        pipelines_.long_block_score = load("qwen35_long_block_score");
        pipelines_.long_block_topk = load("qwen35_long_block_topk");
        pipelines_.long_attention_selected = load("qwen35_long_attention_selected");
        pipelines_.long_block_recall = load("qwen35_long_block_recall");
...
static uint64_t requested_context_bytes() {
    const char* text = std::getenv("QWEN_CONTEXT_GIB");
    if (!text) return uint64_t(kDefaultContext) * kFullLayers * 2u *
        kKvHeads * kHeadDim * sizeof(uint16_t);
    const double gib = std::stod(text);
    if (gib < 0.05 || gib > 16.0)
        throw std::runtime_error("QWEN_CONTEXT_GIB must be 0.05..16");
    return static_cast<uint64_t>(gib * 1024.0 * 1024.0 * 1024.0);
}

static uint64_t context_storage_bytes(uint32_t tokens) {
    return uint64_t(tokens) * kFullLayers * 2u * kKvHeads * kHeadDim *
        sizeof(uint16_t);
}

static uint32_t requested_context_tokens() {
    if (const char* exact = std::getenv("QWEN_CONTEXT_TOKENS")) {
        const uint64_t value = std::stoull(exact);
        if (value < 128 || value > UINT32_MAX)
            throw std::runtime_error("QWEN_CONTEXT_TOKENS must be 128..2^32-1");
        return static_cast<uint32_t>(value);
    }
    const uint64_t budget = requested_context_bytes();
    const uint64_t bytes_per_token = uint64_t(kFullLayers) * 2u *
        kKvHeads * kHeadDim * sizeof(uint16_t);
    const uint64_t tokens = budget / bytes_per_token;
    if (tokens < 128 || tokens > UINT32_MAX)
        throw std::runtime_error("Qwen context budget produces an invalid capacity");
    return static_cast<uint32_t>(tokens);
}
...
public:
    QwenEngine(const Runtime& runtime, const SharedIndex& index,
        ...
        const char* long_mode = std::getenv("QWEN_LONG_MODE");
        long_fast_ = long_mode && (std::strcmp(long_mode, "fast") == 0 ||
                                   std::strcmp(long_mode, "auto") == 0);
        long_auto_ = long_mode && std::strcmp(long_mode, "auto") == 0;
        long_fast_audit_ = long_fast_ &&
            std::getenv("QWEN_LONG_FAST_AUDIT") != nullptr;
        if (const char* value = std::getenv("QWEN_LONG_FAST_BLOCK"))
            long_fast_block_ = static_cast<uint32_t>(std::stoul(value));
        if (const char* value = std::getenv("QWEN_LONG_FAST_CANDIDATES"))
            long_fast_candidates_ = static_cast<uint32_t>(std::stoul(value));
        if (const char* value = std::getenv("QWEN_LONG_FAST_REPRESENTATIVES"))
            long_fast_representatives_ = static_cast<uint32_t>(std::stoul(value));
        if (const char* value = std::getenv("QWEN_LONG_AUTO_CROSSOVER"))
            long_auto_crossover_ = static_cast<uint32_t>(std::stoul(value));
        if (long_fast_) {
            if (long_fast_block_ < 128u || long_fast_block_ > 4096u ||
                (long_fast_block_ & (long_fast_block_ - 1u)) != 0u)
                throw std::runtime_error("QWEN_LONG_FAST_BLOCK must be a power of two in [128,4096]");
            if (long_fast_candidates_ < long_fast_block_ ||
                long_fast_candidates_ / long_fast_block_ > 64u ||
                long_fast_candidates_ % long_fast_block_ != 0u)
                throw std::runtime_error("QWEN_LONG_FAST_CANDIDATES must contain 1..64 complete blocks");
            if (long_fast_representatives_ < 1u || long_fast_representatives_ > 16u)
                throw std::runtime_error("QWEN_LONG_FAST_REPRESENTATIVES must be 1..16");
            long_fast_selected_ = long_fast_candidates_ / long_fast_block_;
        }
        if (const char* chunk = std::getenv("QWEN_LONG_ATTN_CHUNK")) {
            attention_chunk_ = static_cast<uint32_t>(std::stoul(chunk));
            if (attention_chunk_ < 256u || attention_chunk_ > 8192u ||
                (attention_chunk_ & (attention_chunk_ - 1u)) != 0u)
                throw std::runtime_error("QWEN_LONG_ATTN_CHUNK must be a power of two in [256,8192]");
        }
        if (const char* heads = std::getenv("QWEN_LONG_ATTN_GROUP_HEADS")) {
            attention_group_heads_ = static_cast<uint32_t>(std::stoul(heads));
            if (attention_group_heads_ != 8u && attention_group_heads_ != 16u)
                throw std::runtime_error("QWEN_LONG_ATTN_GROUP_HEADS must be 8 or 16");
        }
        attention_chunks_ = (max_context_ + attention_chunk_ - 1u) /
            attention_chunk_;
    ...
    std::vector<uint32_t> generate(const Tokenizer& tokenizer,
        ...
        if (std::getenv("QWEN_LONG_CONTEXT_STRESS")) {
            if (max_context_ <= count + 2u)
                throw std::runtime_error("Qwen context is too small for stress decode");
            const uint32_t target = max_context_ - count;
            expand_context_for_stress(prompt.size() - 1u, target);
            position = target;
            std::cout << "long-context stress decode positions: " << target
                      << ".." << (max_context_ - 1u)
                      << " (replayed real prompt K/V; decode-performance mode)\n";
        }
    ...
    uint64_t ram_bytes() const {
        return host_cache_.committed_bytes() +
               ...
               context_storage_bytes(max_context_);
    }
    ...
    uint64_t context_bytes() const { return context_storage_bytes(max_context_); }
    uint64_t long_fast_recalled() {
        if (!long_fast_audit_) return 0;
        invalidate_buffer(runtime_, long_fast_recall_);
        return static_cast<const uint32_t*>(long_fast_recall_.mapped)[0];
    }
    uint64_t long_fast_total() {
        if (!long_fast_audit_) return 0;
        invalidate_buffer(runtime_, long_fast_recall_);
        return static_cast<const uint32_t*>(long_fast_recall_.mapped)[1];
    }
    bool long_fast_enabled() const { return long_fast_; }
    bool long_fast_audited() const { return long_fast_audit_; }
    uint32_t long_fast_block() const { return long_fast_block_; }
    uint32_t long_fast_candidates() const { return long_fast_candidates_; }
    ...
    double context_fill_seconds() const { return context_fill_seconds_; }
...
private:
    ...
    void expand_context_for_stress(uint32_t source_position,
        ...
        if (source_position >= target_position || target_position >= max_context_)
            throw std::runtime_error("Invalid Qwen context stress range");
        const auto started = std::chrono::steady_clock::now();
        const uint64_t token_bytes = uint64_t(kKvHeads) * kHeadDim *
            sizeof(uint16_t);
        const uint64_t plane_bytes = uint64_t(max_context_) * token_bytes;
        for (Buffer& cache : kv_caches_) {
            invalidate_buffer(runtime_, cache);
            auto* bytes = static_cast<uint8_t*>(cache.mapped);
            for (uint32_t plane = 0; plane < 2u; ++plane) {
                uint8_t* base = bytes + uint64_t(plane) * plane_bytes;
                const uint64_t source_records = uint64_t(source_position) + 1u;
                uint64_t pattern_records = source_records;
                if (const char* tail = std::getenv("QWEN_LONG_STRESS_TAIL_TOKENS"))
                    pattern_records = std::clamp<uint64_t>(std::stoull(tail), 1u,
                                                           source_records);
                const uint64_t pattern_start = source_records - pattern_records;
                uint8_t* begin = base + source_records * token_bytes;
                const uint64_t records = target_position - source_records;
                if (!records) continue;
                uint64_t filled = 0;
                while (filled < records) {
                    const uint64_t copy_records = std::min(pattern_records,
                                                           records - filled);
                    std::memcpy(begin + filled * token_bytes,
                                base + pattern_start * token_bytes,
                                static_cast<size_t>(copy_records * token_bytes));
                    filled += copy_records;
                }
            }
            dsv4::flush_buffer_range(runtime_, cache, 0, cache.size);
        }
        context_fill_seconds_ = std::chrono::duration<double>(
            std::chrono::steady_clock::now() - started).count();
    }
    ...
    void allocate_buffers() {
        ...
        recurrent_state_ = device(uint64_t(kLinearLayers) *
                                  ...
                                  kLinearHeadDim * 4);
        const uint64_t layer_context_bytes = uint64_t(max_context_) * 2u *
            kKvHeads * kHeadDim * sizeof(uint16_t);
        for (Buffer& cache : kv_caches_)
            cache = dsv4::create_host_buffer_uninitialized(
                runtime_, layer_context_bytes);
        attention_partial_ = device(uint64_t(attention_chunks_) *
            kAttentionHeads * kAttentionPartialStride * sizeof(float));
        if (long_fast_) {
            const uint64_t blocks = (uint64_t(max_context_) +
                long_fast_block_ - 1u) / long_fast_block_;
            long_fast_scores_ = device(2u * blocks * sizeof(float));
            long_fast_blocks_ = device(2u * 64u * sizeof(uint32_t));
            if (long_fast_audit_) {
                long_fast_exact_scores_ = device(2u * blocks * sizeof(float));
                long_fast_exact_blocks_ = device(2u * 64u * sizeof(uint32_t));
                long_fast_recall_ = create_buffer(runtime_, 4u * sizeof(uint32_t));
                std::memset(long_fast_recall_.mapped, 0, 4u * sizeof(uint32_t));
                flush_buffer(runtime_, long_fast_recall_);
            }
        }
        if (verify2_enabled_) {
    ...
    struct LayerSets {
        ...
        VkDescriptorSet attention{}, attention_reduce{}, head_gate{};
        VkDescriptorSet block_score{}, block_topk{}, selected_attention{};
        VkDescriptorSet exact_block_score{}, exact_block_topk{}, block_recall{};
        VkDescriptorSet context_quant{}, attention_out{};
    ...
    void build_sets() {
        ...
        for (uint32_t layer = 0; layer < kLayers; ++layer) {
            ...
            if (full_attention(layer)) {
                ...
                Buffer& layer_cache = kv_caches_[full_index(layer)];
                sets.qk = kernels_.set({whole(qgate_), whole(key_), qnorm.data,
                                        knorm.data, whole(layer_cache)});
                sets.store_value = kernels_.set({whole(value_), whole(layer_cache)});
                sets.attention = kernels_.set(
                    {whole(qgate_), whole(layer_cache), whole(attention_partial_)});
                sets.attention_reduce = kernels_.set(
                    {whole(attention_partial_), whole(context_)});
                if (long_fast_) {
                    sets.block_score = kernels_.set({whole(qgate_), whole(layer_cache),
                                                     whole(long_fast_scores_)});
                    sets.block_topk = kernels_.set({whole(long_fast_scores_),
                                                    whole(long_fast_blocks_)});
                    sets.selected_attention = kernels_.set({whole(qgate_),
                        whole(layer_cache), whole(long_fast_blocks_),
                        whole(attention_partial_)});
                    if (long_fast_audit_) {
                        sets.exact_block_score = kernels_.set({whole(qgate_),
                            whole(layer_cache), whole(long_fast_exact_scores_)});
                        sets.exact_block_topk = kernels_.set({
                            whole(long_fast_exact_scores_),
                            whole(long_fast_exact_blocks_)});
                        sets.block_recall = kernels_.set({whole(long_fast_blocks_),
                            whole(long_fast_exact_blocks_), whole(long_fast_recall_)});
                    }
                }
    ...
    void record_attention(VkCommandBuffer command, uint32_t layer,
        ...
        if (full_attention(layer)) {
            ...
            kernels_.dispatch(command, kernels_.p().store_value,
                              ...
                              (kKvHeads * kHeadDim / 2u + 63) / 64);
            compute_barrier(command);
            const bool use_fast = long_fast_ &&
                (!long_auto_ || position + 1u >= long_auto_crossover_) &&
                position + 1u > long_fast_candidates_;
            uint32_t chunks = 0u;
            if (use_fast) {
                const uint32_t blocks = (position + long_fast_block_) /
                    long_fast_block_;
                const uint32_t selected = std::min(blocks, long_fast_selected_);
                push = {position, max_context_, long_fast_block_,
                        long_fast_representatives_};
                kernels_.dispatch(command, kernels_.p().long_block_score,
                    sets.block_score, &push, blocks * kKvHeads);
                compute_barrier(command);
                push = {blocks, selected, 0u, 0u};
                kernels_.dispatch(command, kernels_.p().long_block_topk,
                    sets.block_topk, &push, kKvHeads);
                compute_barrier(command);
                if (long_fast_audit_) {
                    push = {position, max_context_, long_fast_block_, 0u};
                    kernels_.dispatch(command, kernels_.p().long_block_score,
                        sets.exact_block_score, &push, blocks * kKvHeads);
                    compute_barrier(command);
                    push = {blocks, selected, 0u, 0u};
                    kernels_.dispatch(command, kernels_.p().long_block_topk,
                        sets.exact_block_topk, &push, kKvHeads);
                    compute_barrier(command);
                    push = {selected, 0u, 0u, 0u};
                    kernels_.dispatch(command, kernels_.p().long_block_recall,
                        sets.block_recall, &push,
                        (kKvHeads * selected + 63u) / 64u);
                    compute_barrier(command);
                }
                push = {position, max_context_, long_fast_block_, selected};
                kernels_.dispatch(command, kernels_.p().long_attention_selected,
                    sets.selected_attention, &push, selected * kKvHeads);
                chunks = selected;
            } else {
                chunks = (position + attention_chunk_) / attention_chunk_;
                push = {position, max_context_, attention_chunk_, chunks};
                kernels_.dispatch(command, kernels_.p().attention, sets.attention,
                                  &push, chunks * (kAttentionHeads / attention_group_heads_));
            }
            compute_barrier(command);
            push = {chunks, kAttentionHeads, 0, 0};
    ...
    void destroy_all() {
        ...
        for (Buffer& cache : kv_caches_) destroy_buffer(runtime_, cache);
        for (Buffer* buffer : {&long_fast_recall_, &long_fast_exact_blocks_,
                               &long_fast_exact_scores_, &long_fast_blocks_,
                               &long_fast_scores_, &attention_partial_, &recurrent_state_,
    ...
    std::array<Buffer, kFullLayers> kv_caches_{};
    Buffer attention_partial_{};
    Buffer long_fast_scores_{}, long_fast_blocks_{};
    Buffer long_fast_exact_scores_{}, long_fast_exact_blocks_{}, long_fast_recall_{};
    ...
    uint32_t attention_chunks_ =
        (kDefaultContext + kAttentionChunk - 1u) / kAttentionChunk;
    uint32_t attention_chunk_ = 512u;
    uint32_t attention_group_heads_ = 16u;
    bool long_fast_ = false, long_auto_ = false, long_fast_audit_ = false;
    uint32_t long_fast_block_ = 512u, long_fast_candidates_ = 16384u;
    uint32_t long_fast_representatives_ = 4u, long_fast_selected_ = 32u;
    uint32_t long_auto_crossover_ = 65536u;
    ...
    double context_fill_seconds_ = 0;
};
...
int qwen35_cli_main(int argc, char** argv) {
    ...
    try {
        ...
        uint64_t long_fast_recalled = 0, long_fast_total = 0;
        bool long_fast_enabled = false, long_fast_audited = false;
        uint32_t long_fast_block = 0, long_fast_candidates = 0;
        {
            ...
            std::cout << " / " << engine.host_slots() << " global\n"
                      ...
                      << "long context capacity: " << engine.max_context()
                      << " tokens, BF16 host K/V "
                      << double(engine.context_bytes()) / double(1ull << 30)
                      << " GiB\n";
            ...
            context_fill = engine.context_fill_seconds();
            ...
            context_capacity = engine.max_context();
            context_bytes = engine.context_bytes();
            long_fast_recalled = engine.long_fast_recalled();
            long_fast_total = engine.long_fast_total();
            long_fast_enabled = engine.long_fast_enabled();
            long_fast_audited = engine.long_fast_audited();
            long_fast_block = engine.long_fast_block();
            long_fast_candidates = engine.long_fast_candidates();
        }
        ...
        std::cout << "\ndecode throughput: "
                  ...
                  << "context stress fill seconds: " << context_fill << '\n'
                  << "peak explicit host model RAM GiB: "
                  << double(ram) / double(1ull << 30) << " (capacity "
                  << host_slots << " records; context " << context_capacity
                  << " tokens / " << double(context_bytes) / double(1ull << 30)
                  << " GiB)\n"
                  << "peak device allocation estimate GiB: "
                  << double(vram) / double(1ull << 30) << '\n';
        if (long_fast_enabled) {
            const double recall = long_fast_total ?
                100.0 * double(long_fast_recalled) / double(long_fast_total) : 0.0;
            std::cout << "Qwen fast long block/candidates: "
                      << long_fast_block << '/' << long_fast_candidates << '\n'
                      << "Qwen fast selected-block recall: "
                      << long_fast_recalled << '/' << long_fast_total << " ("
                      << recall << "%)"

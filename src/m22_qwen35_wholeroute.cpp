...
public:
    ...
    bool batch_item_ready(const AsyncBatch& batch, uint32_t index) const {
        if (!batch.active || index >= batch.count || !batch.pending[index])
            return false;
        return WaitForSingleObject(batch.events[index], 0) == WAIT_OBJECT_0;
    }

    void wait_batch_item(AsyncBatch& batch, uint32_t index) {
        if (!batch.active || index >= batch.count || !batch.pending[index])
            throw std::runtime_error("Invalid speculative Qwen read item");
        if (WaitForSingleObject(batch.events[index], 10000) != WAIT_OBJECT_0)
            throw std::runtime_error("Speculative Qwen read timed out");
        DWORD transferred = 0;
        if (!GetOverlappedResult(file_, &batch.operations[index], &transferred,
                                 FALSE) || transferred != kExpertRecordBytes)
            throw std::runtime_error("Speculative Qwen read failed");
        batch.pending[index] = false;
        --batch.remaining;
    }

    void cancel_batch(AsyncBatch& batch) {
        if (!batch.active) return;
        for (uint32_t index = 0; index < batch.count; ++index) {
            if (!batch.pending[index]) continue;
            CancelIoEx(file_, &batch.operations[index]);
            WaitForSingleObject(batch.events[index], 10000);
            DWORD transferred = 0;
            GetOverlappedResult(file_, &batch.operations[index], &transferred,
                                FALSE);
            batch.pending[index] = false;
        }
        for (uint32_t index = 0; index < batch.count; ++index)
            if (batch.events[index]) CloseHandle(batch.events[index]);
        batch = {};
    }
...
class WholeRoutePredictor {
public:
    struct Prediction {
        std::array<std::array<uint8_t, kTopK>, kLayers> experts{};
        std::array<std::array<uint8_t, kTopK>, kLayers> confidence{};
    };

    explicit WholeRoutePredictor(const std::filesystem::path& path) {
        struct Header {
            char magic[8];
            uint32_t version, layers, experts, topk, records, record_bytes;
            uint64_t reserved[8];
        };
        ...
        if (!input || std::memcmp(header.magic, "Q35RTP1\0", 8) != 0 ||
            header.version != 1 || header.layers != kLayers ||
            header.experts != kExperts || header.topk != kTopK ||
            header.record_bytes != 776)
            throw std::runtime_error("Unsupported Qwen whole-route predictor");
        input.read(reinterpret_cast<char*>(fallback_.experts.data()),
                   kLayers * kTopK);
        input.read(reinterpret_cast<char*>(fallback_.confidence.data()),
                   kLayers * kTopK);
        for (uint32_t index = 0; index < header.records; ++index) {
            uint32_t token = 0, reserved = 0;
            Prediction prediction{};
            input.read(reinterpret_cast<char*>(&token), sizeof(token));
            input.read(reinterpret_cast<char*>(prediction.experts.data()),
                       kLayers * kTopK);
            input.read(reinterpret_cast<char*>(prediction.confidence.data()),
                       kLayers * kTopK);
            input.read(reinterpret_cast<char*>(&reserved), sizeof(reserved));
            if (!input) throw std::runtime_error("Truncated Qwen route predictor");
            table_.emplace(token, std::move(prediction));
        }
    }

    const Prediction& get(uint32_t token) const {
        const auto found = table_.find(token);
        return found == table_.end() ? fallback_ : found->second;
    }

private:
    Prediction fallback_{};
    std::unordered_map<uint32_t, Prediction> table_;
};
...
public:
    ...
    bool contains(uint32_t layer, uint32_t expert) const {
        return locations_.at(uint64_t(layer) * kExperts + expert) >= 0;
    }

    bool install_prefetched(uint32_t layer, uint32_t expert,
                            const void* source) {
        const uint32_t key = layer * kExperts + expert;
        if (locations_[key] >= 0) return false;
        uint32_t victim = UINT32_MAX;
        for (uint32_t slot = 0; slot < slots_; ++slot) {
            bool replace = victim == UINT32_MAX || entries_[slot].key < 0;
            if (!replace && entries_[victim].key >= 0) {
        ...
        if (victim == UINT32_MAX)
            throw std::runtime_error("No Qwen speculative RAM-cache victim");
        Entry& entry = entries_[victim];
        if (entry.key >= 0) locations_[entry.key] = -1;
        uint8_t* destination = base_ + uint64_t(victim) * kExpertRecordBytes;
        if (!entry.committed) {
            if (!VirtualAlloc(destination, kExpertRecordBytes, MEM_COMMIT,
                              PAGE_READWRITE))
                throw std::runtime_error("Qwen speculative cache commit failed");
            entry.committed = true;
            ++committed_;
        }
        std::memcpy(destination, source, kExpertRecordBytes);
        entry.key = static_cast<int32_t>(key);
        entry.age = ++clock_;
        locations_[key] = static_cast<int32_t>(victim);
        frequency_[key] = std::max<uint32_t>(frequency_[key], 1u);
        return true;
    }
...
public:
    ...
    bool contains(uint32_t layer_index, uint32_t expert) const {
        const Layer& layer = layers_.at(layer_index);
        return std::any_of(layer.entries.begin(), layer.entries.end(),
            [&](const Entry& entry) { return entry.expert == int32_t(expert); });
    }
...
public:
    QwenEngine(const Runtime& runtime, const SharedIndex& index,
        ...
        const char* predictor_path =
            std::getenv("QWEN_WHOLE_ROUTE_PREDICTOR");
        quasi_hidden_ = predictor_path && *predictor_path;
        if (quasi_hidden_)
            route_predictor_ = std::make_unique<WholeRoutePredictor>(
        ...
        for (Buffer& buffer : staging_)
            buffer = dsv4::create_host_buffer_uninitialized(runtime_,
                                                             kExpertRecordBytes);
        if (quasi_hidden_) {
            speculative_staging_.resize(kTopK);
            for (Buffer& buffer : speculative_staging_)
    ...
    ~QwenEngine() {
        expert_file_.cancel_batch(speculative_reads_);
        for (Buffer& buffer : speculative_staging_)
    ...
    uint64_t disk_bytes() const {
        return host_cache_.disk_bytes() +
               speculative_submitted_ * kExpertRecordBytes;
    }
    ...
    uint64_t ram_bytes() const {
        return host_cache_.committed_bytes() +
               uint64_t(staging_.size() + speculative_staging_.size()) *
                   kExpertRecordBytes;
    }
    ...
    uint64_t prediction_total() const { return prediction_total_; }
    uint64_t prediction_hits() const { return prediction_hits_; }
    uint64_t prediction_ssd_misses() const { return ssd_miss_total_; }
    uint64_t prediction_ssd_hits() const { return ssd_miss_predicted_; }
    uint64_t speculative_submitted() const { return speculative_submitted_; }
    uint64_t speculative_useful() const { return speculative_useful_; }
    uint64_t speculative_wasted() const { return speculative_wasted_; }
    uint64_t ready_before_deadline() const { return ready_before_deadline_; }
...
private:
    ...
    void allocate_buffers() {
        ...
        if (quasi_hidden_) {
            predict_normalized_ = device(uint64_t(kDim) * 4);
            predict_quant_ = device(4224ull * 4);
            predict_logits_ = device(uint64_t(kExperts) * 4);
            predict_routing_ = create_buffer(runtime_, 16u * sizeof(uint32_t));
        }
        if (verify2_enabled_) {
    ...
    struct LayerSets {
        ...
        VkDescriptorSet predict_norm{}, predict_quant{};
        VkDescriptorSet predict_router_gemv{}, predict_router{};
    ...
    void build_sets() {
        ...
        for (uint32_t layer = 0; layer < kLayers; ++layer) {
            ...
            if (quasi_hidden_) {
                sets.predict_norm = kernels_.set(
                    {whole(hidden_), post_norm.data, whole(predict_normalized_)});
                sets.predict_quant = kernels_.set(
                    {whole(predict_normalized_), whole(predict_quant_)});
                sets.predict_router_gemv = kernels_.set({
                    whole(predict_quant_), router.data, router.auxiliary,
                    whole(predict_logits_)});
                sets.predict_router = kernels_.set(
                    {whole(predict_logits_), whole(predict_routing_)});
            }
    ...
    void reset_decode_metrics() {
        ...
        prediction_total_ = prediction_hits_ = ssd_miss_total_ = 0;
        ssd_miss_predicted_ = speculative_submitted_ = 0;
        speculative_useful_ = speculative_wasted_ = ready_before_deadline_ = 0;
        prediction_seconds_ = 0.0;
    }
    ...
    uint32_t run_token(uint32_t token, uint32_t position) {
        ...
        *static_cast<uint32_t*>(token_.mapped) = token;
        ...
        schedule_whole_route_prefetch(token);
        uint64_t signal = compute_.submit([&](VkCommandBuffer command) {
            Push push{kVocabulary, kDim, kDim / 4, 0};
        ...
        for (uint32_t layer = 0; layer < kLayers; ++layer) {
            ...
            consume_quasi_prefetch(layer, experts);
    ...
    void schedule_whole_route_prefetch(uint32_t token) {
        if (!quasi_hidden_ || !decode_phase_ || !route_predictor_) return;
        expert_file_.cancel_batch(speculative_reads_);
        const auto started = std::chrono::steady_clock::now();
        const WholeRoutePredictor::Prediction& prediction =
            route_predictor_->get(token);
        struct Candidate { uint32_t layer, expert, confidence; };
        std::vector<Candidate> candidates;
        for (uint32_t layer = 0; layer < kLayers; ++layer) {
            whole_predicted_[layer] = prediction.experts[layer][0];
            whole_confidence_[layer] = prediction.confidence[layer][0];
            if (layer < 4 || whole_confidence_[layer] < 192 ||
                device_cache_.contains(layer, whole_predicted_[layer]) ||
                host_cache_.contains(layer, whole_predicted_[layer]))
                continue;
            candidates.push_back(
                {layer, whole_predicted_[layer], whole_confidence_[layer]});
        }
        ...
        speculative_count_ = 0;
        for (const Candidate& candidate : candidates) {
            if (speculative_count_ == kTopK) break;
            const uint32_t index = speculative_count_++;
            speculative_layers_[index] = candidate.layer;
            speculative_experts_[index] = candidate.expert;
            offsets.push_back(expert_file_.offset(candidate.layer,
                                                  candidate.expert));
            destinations.push_back(
                speculative_staging_[index].mapped);
            ranks.push_back(index);
        }
        if (speculative_count_) {
            expert_file_.begin_batch(offsets, destinations, ranks,
                                     speculative_reads_);
            speculative_submitted_ += speculative_count_;
        }
        prediction_seconds_ += std::chrono::duration<double>(
            std::chrono::steady_clock::now() - started).count();
    }

    void schedule_quasi_prefetch(uint32_t) {}

    void consume_quasi_prefetch(
        uint32_t layer, const std::array<uint32_t, kTopK>& authoritative) {
        if (!quasi_hidden_ || !decode_phase_) return;
        ++prediction_total_;
        if (std::find(authoritative.begin(), authoritative.end(),
                      whole_predicted_[layer]) != authoritative.end())
            ++prediction_hits_;

        for (uint32_t expert : authoritative) {
            if (device_cache_.contains(layer, expert) ||
                host_cache_.contains(layer, expert))
                continue;
            ++ssd_miss_total_;
            if (whole_predicted_[layer] == expert)
                ++ssd_miss_predicted_;
        }

        for (uint32_t index = 0; index < speculative_count_; ++index) {
            if (speculative_layers_[index] != layer) continue;
            const bool useful =
                std::find(authoritative.begin(), authoritative.end(),
                          speculative_experts_[index]) != authoritative.end();
            if (useful) {
                ++speculative_useful_;
                if (expert_file_.batch_item_ready(speculative_reads_, index))
                    ++ready_before_deadline_;
                expert_file_.wait_batch_item(speculative_reads_, index);
                if (!host_cache_.contains(layer, speculative_experts_[index])) {
                    host_cache_.install_prefetched(
                        layer, speculative_experts_[index],
                        speculative_staging_[index].mapped);
                    host_copy_bytes_ += kExpertRecordBytes;
                }
            } else {
            ...
            speculative_layers_[index] = UINT32_MAX;
        }
    }

    void finish_whole_route_prefetch() {
        if (!quasi_hidden_ || !decode_phase_) return;
        for (uint32_t index = 0; index < speculative_count_; ++index)
            if (speculative_layers_[index] != UINT32_MAX)
                ++speculative_wasted_;
        expert_file_.cancel_batch(speculative_reads_);
        speculative_count_ = 0;
    }
    ...
    void destroy_all() {
        if (quasi_hidden_) {
            destroy_buffer(runtime_, predict_routing_);
            destroy_buffer(runtime_, predict_logits_);
            destroy_buffer(runtime_, predict_quant_);
            destroy_buffer(runtime_, predict_normalized_);
        }
        if (verify2_enabled_) {
    ...
    std::vector<Buffer> speculative_staging_;
    Buffer predict_normalized_{}, predict_quant_{}, predict_logits_{};
    Buffer predict_routing_{};
    std::unique_ptr<WholeRoutePredictor> route_predictor_;
    ExpertFile::AsyncBatch speculative_reads_{};
    std::array<uint32_t, kTopK> speculative_experts_{};
    std::array<uint32_t, kTopK> speculative_layers_{};
    std::array<uint32_t, kLayers> whole_predicted_{};
    std::array<uint8_t, kLayers> whole_confidence_{};
    uint32_t speculative_count_ = 0, speculative_layer_ = UINT32_MAX;
    ...
    bool quasi_hidden_ = false, decode_phase_ = false;
    ...
    uint64_t prediction_total_ = 0, prediction_hits_ = 0;
    uint64_t ssd_miss_total_ = 0, ssd_miss_predicted_ = 0;
    uint64_t speculative_submitted_ = 0, speculative_useful_ = 0;
    uint64_t speculative_wasted_ = 0, ready_before_deadline_ = 0;
    double decode_seconds_ = 0, pre_seconds_ = 0;
    double acquisition_seconds_ = 0, expert_seconds_ = 0;
    double prediction_seconds_ = 0;
};
...
int qwen35_cli_main(int argc, char** argv) {
    ...
    try {
        ...
        uint64_t prediction_total = 0, prediction_hits = 0;
        uint64_t prediction_ssd_total = 0, prediction_ssd_hits = 0;
        uint64_t speculative_submitted = 0, speculative_useful = 0;
        uint64_t speculative_wasted = 0, ready_before_deadline = 0;
        ...
        uint32_t host_slots = 0;
        {
            ...
            prediction_total = engine.prediction_total();
            prediction_hits = engine.prediction_hits();
            prediction_ssd_total = engine.prediction_ssd_misses();
            prediction_ssd_hits = engine.prediction_ssd_hits();
            speculative_submitted = engine.speculative_submitted();
            speculative_useful = engine.speculative_useful();
            speculative_wasted = engine.speculative_wasted();
            ready_before_deadline = engine.ready_before_deadline();
        ...
        std::cout << "\ndecode throughput: "
                  ...
                  << '\n'
                  << "whole-route Top-1 precision: "
                  << prediction_hits << '/' << prediction_total << " ("
                  << (prediction_total ? 100.0 * prediction_hits /
                        prediction_total : 0.0) << "%)\n"
                  << "whole-route SSD-miss recall: "
                  << prediction_ssd_hits << '/' << prediction_ssd_total << " ("
                  << (prediction_ssd_total ? 100.0 * prediction_ssd_hits /
                        prediction_ssd_total : 0.0) << "%)\n"
                  << "whole-route submitted/useful/wasted/ready: "
                  << speculative_submitted << '/' << speculative_useful << '/'
                  << speculative_wasted << '/' << ready_before_deadline << '\n'
                  << "whole-route predictor wall s: " << prediction_seconds
                  << '\n'

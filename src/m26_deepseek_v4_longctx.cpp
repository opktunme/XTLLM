...
constexpr uint32_t kMaximumContext = 1048576;
...
struct Dsv4Pipelines {
    ...
    VkPipeline long_context_scan = VK_NULL_HANDLE;
    VkPipeline long_dynamic_rope = VK_NULL_HANDLE;
    VkPipeline long_inverse_rope = VK_NULL_HANDLE;
    VkPipeline long_compress_decode = VK_NULL_HANDLE;
    VkPipeline long_index_query = VK_NULL_HANDLE;
    VkPipeline long_index_score = VK_NULL_HANDLE;
    VkPipeline long_fp4_expand = VK_NULL_HANDLE;
    VkPipeline long_index_score_i8 = VK_NULL_HANDLE;
    VkPipeline long_index_coarse = VK_NULL_HANDLE;
    VkPipeline long_index_candidates = VK_NULL_HANDLE;
    VkPipeline long_topk = VK_NULL_HANDLE;
    VkPipeline long_candidate_topk = VK_NULL_HANDLE;
    VkPipeline long_recall = VK_NULL_HANDLE;
    VkPipeline long_sparse_attention = VK_NULL_HANDLE;
...
public:
    Dsv4KernelLibrary(const Runtime& runtime,
        ...
        pipelines_.long_context_scan = load("dsv4_long_context_scan");
        pipelines_.long_dynamic_rope = load("dsv4_long_dynamic_rope");
        pipelines_.long_inverse_rope = load("dsv4_long_inverse_rope");
        pipelines_.long_compress_decode = load("dsv4_long_compress_decode");
        pipelines_.long_index_query = load("dsv4_long_index_query");
        pipelines_.long_index_score = load(std::getenv("DSV4_INDEX_SCORE_SHADER") ?
            std::getenv("DSV4_INDEX_SCORE_SHADER") : "dsv4_long_index_score");
        pipelines_.long_fp4_expand = load("dsv4_long_fp4_expand");
        pipelines_.long_index_score_i8 = load(
            std::getenv("DSV4_INDEX_SCORE_I8_SHADER") ?
            std::getenv("DSV4_INDEX_SCORE_I8_SHADER") :
            "dsv4_long_index_score_i8");
        pipelines_.long_index_coarse = load("dsv4_long_index_coarse");
        pipelines_.long_index_candidates = load("dsv4_long_index_candidates");
        pipelines_.long_topk = load("dsv4_long_topk");
        pipelines_.long_candidate_topk = load("dsv4_long_candidate_topk");
        pipelines_.long_recall = load("dsv4_long_recall");
        pipelines_.long_sparse_attention = load("dsv4_long_sparse_attention");
...
public:
    ...
    uint32_t run_token(uint32_t token, uint32_t position,
                       ...
                       const SharedPreRecorder* record_shared_pre = nullptr,
                       const ProgressiveExpertRecorders* progressive = nullptr) {
        const bool long_context = std::getenv("DSV4_LONG_CONTEXT_STRESS") != nullptr ||
                                  std::getenv("DSV4_LONG_CONTEXT_REAL") != nullptr;
        if ((!long_context && position >= kShortContext) ||
            (long_context && position >= kMaximumContext))
            throw std::runtime_error("DeepSeek position exceeds the enabled context path");
    ...
    std::vector<uint32_t> generate(const Tokenizer& tokenizer,
        ...
        const bool stress_context =
            std::getenv("DSV4_LONG_CONTEXT_STRESS") != nullptr;
        const bool long_context = stress_context ||
            std::getenv("DSV4_LONG_CONTEXT_REAL") != nullptr;
        if (!long_context && prompt.size() + maximum_new_tokens > kShortContext)
        ...
        uint32_t stress_position = static_cast<uint32_t>(prompt.size());
        if (stress_context) {
            const char* budget_text = std::getenv("DSV4_CONTEXT_GIB");
            if (!budget_text)
                throw std::runtime_error("Long DeepSeek context budget is missing");
            const uint64_t bytes = static_cast<uint64_t>(
                std::stod(budget_text) * double(1ull << 30));
            uint32_t capacity = static_cast<uint32_t>(std::min<uint64_t>(
                bytes / 5893ull, kMaximumContext));
            if (const char* exact = std::getenv("DSV4_CONTEXT_CAPACITY"))
                capacity = static_cast<uint32_t>(std::stoul(exact));
            if (capacity <= maximum_new_tokens + kWindow)
                throw std::runtime_error("DeepSeek context stress capacity is too small");
            stress_position = capacity - maximum_new_tokens;
            std::cout << "long-context learned decode positions: "
                      << stress_position << ".." << capacity - 1u << '\n';
        }
        cache_hits_ = 0;
        cache_misses_ = 0;
        ...
        for (uint32_t item = 0; item < maximum_new_tokens; ++item) {
            ...
            const uint32_t position = stress_context ?
                stress_position + item :
                static_cast<uint32_t>(prompt.size() + item);
...
public:
    DeepSeekProgram(const Runtime& runtime, ExecutorScaffold& executor,
        ...
        if (const char* rows = std::getenv("DSV4_INDEX_SCORE_ROWS")) {
            index_score_rows_ = static_cast<uint32_t>(std::stoul(rows));
            if (index_score_rows_ != 1u && index_score_rows_ != 2u &&
                index_score_rows_ != 4u && index_score_rows_ != 8u &&
                index_score_rows_ != 16u)
                throw std::runtime_error("DSV4_INDEX_SCORE_ROWS must be 1,2,4,8,16");
        }
        const char* long_mode = std::getenv("DSV4_LONG_MODE");
        long_fast_ = std::getenv("DSV4_LONG_FAST") != nullptr ||
            (long_mode && (std::strcmp(long_mode, "fast") == 0 ||
                           std::strcmp(long_mode, "auto") == 0));
        long_auto_ = long_mode && std::strcmp(long_mode, "auto") == 0;
        if (const char* value = std::getenv("DSV4_LONG_AUTO_CROSSOVER"))
            long_auto_crossover_ = static_cast<uint32_t>(std::stoul(value));
        long_fast_audit_ = long_fast_ &&
            std::getenv("DSV4_LONG_FAST_AUDIT") != nullptr;
        index_i8_ = std::getenv("DSV4_INDEX_INT8") != nullptr &&
            (!long_fast_ || long_auto_ || long_fast_audit_);
        if (const char* value = std::getenv("DSV4_LONG_FAST_BLOCK"))
            long_fast_block_ = static_cast<uint32_t>(std::stoul(value));
        if (const char* value = std::getenv("DSV4_LONG_FAST_CANDIDATES"))
            long_fast_candidates_ = static_cast<uint32_t>(std::stoul(value));
        if (const char* value = std::getenv("DSV4_LONG_FAST_REPRESENTATIVES"))
            long_fast_representatives_ = static_cast<uint32_t>(std::stoul(value));
        if (long_fast_) {
            if (long_fast_block_ < 32u || long_fast_block_ > 1024u ||
                (long_fast_block_ & (long_fast_block_ - 1u)) != 0u)
                throw std::runtime_error("DSV4_LONG_FAST_BLOCK must be a power of two in [32,1024]");
            if (long_fast_candidates_ < 512u || long_fast_candidates_ > 262144u ||
                long_fast_candidates_ % long_fast_block_ != 0u)
                throw std::runtime_error("DSV4_LONG_FAST_CANDIDATES must be 512..262144 and block-aligned");
            if (long_fast_representatives_ < 1u || long_fast_representatives_ > 8u)
                throw std::runtime_error("DSV4_LONG_FAST_REPRESENTATIVES must be 1..8");
        }
        ratio4_slot_.fill(-1);
        for (uint32_t layer = 0; layer < kLayers; ++layer) {
        ...
        indices_ = create_buffer(runtime_, 640 * sizeof(uint32_t));
    ...
    ~DeepSeekProgram() {
        destroy_buffer(runtime_, long_context_checksum_);
        for (Buffer& buffer : long_context_buffers_)
            destroy_buffer(runtime_, buffer);
        for (LongLayerState& state : long_layers_) {
            destroy_buffer(runtime_, state.index_cache_i8);
            destroy_buffer(runtime_, state.index_cache);
            destroy_buffer(runtime_, state.main_cache);
        }
        destroy_buffer(runtime_, long_index_scores_);
        destroy_buffer(runtime_, long_recall_metrics_);
        destroy_buffer(runtime_, long_exact_indices_);
        destroy_buffer(runtime_, long_candidate_positions_);
        destroy_buffer(runtime_, long_candidate_scores_);
        destroy_buffer(runtime_, long_block_indices_);
        destroy_buffer(runtime_, long_coarse_scores_);
        destroy_buffer(runtime_, long_index_weights_);
        destroy_buffer(runtime_, long_index_query_packed_);
        destroy_buffer(runtime_, long_index_query_i8_);
        destroy_buffer(runtime_, long_index_query_);
        destroy_buffer(runtime_, long_index_score_projection_);
        destroy_buffer(runtime_, long_index_kv_projection_);
        destroy_buffer(runtime_, long_score_projection_);
        destroy_buffer(runtime_, long_kv_projection_);
        destroy_buffer(runtime_, long_index_state_);
        destroy_buffer(runtime_, long_main_state_);
    ...
    void record_embedding(VkCommandBuffer command, uint32_t, uint32_t position) {
        ...
        current_position_ = position;
        if (position == contiguous_context_tokens_) ++contiguous_context_tokens_;
    ...
    void record_pre(VkCommandBuffer command, uint32_t layer, uint32_t) {
        ...
        if (layer == 0u) prepare_stress_history();
        current_command_ = command;
        ...
        if (long_context_enabled_) {
            LongLayerState& state = long_layers_[layer];
            if (state.ratio) {
                const uint32_t rows = state.ratio == 4u ? 1024u : 512u;
                gemv(command, sets.long_main_wkv, rows, kDimension, 1024u, 0u);
                gemv(command, sets.long_main_gate, rows, kDimension, 1024u, 0u);
            }
            if (state.ratio == 4u) {
                gemv(command, sets.long_index_wkv, 256u, kDimension, 1024u, 0u);
                gemv(command, sets.long_index_gate, 256u, kDimension, 1024u, 0u);
                gemv(command, sets.long_index_weights, 64u, kDimension, 1024u, 0u);
            }
        }
        if (sets.ratio4 && current_position_ < kShortContext) {
        ...
        if (long_context_enabled_ && long_layers_[layer].ratio == 4u)
            gemv(command, sets.long_index_wq, 8192u, 1024u, 256u, 0u);
        compute_barrier(command);
        const bool use_long_attention = long_context_enabled_ &&
                                        current_position_ >= kShortContext;
        if (use_long_attention) {
            const std::array<uint32_t, 4> qpush{
                rms_bits(), current_position_,
                index_.compression_ratio(layer) ? 1u : 0u, kHeads};
            dispatch(sets.long_query_rope, kernels_.pipelines().long_dynamic_rope,
                     qpush.data(), kHeads);
            const std::array<uint32_t, 4> kvpush{
                rms_bits(), current_position_,
                index_.compression_ratio(layer) ? 1u : 0u, 1u};
            dispatch(sets.long_kv_rope, kernels_.pipelines().long_dynamic_rope,
                     kvpush.data(), 1u);
            compute_barrier(command);
        } else if (rms_rope_fused_) {
        ...
        copy_kv(command, layer, current_position_ % kWindow);

        if (long_context_enabled_ && long_layers_[layer].ratio) {
            LongLayerState& state = long_layers_[layer];
            const uint32_t flags = state.ratio == 4u ? 1u : 0u;
            const std::array<uint32_t, 4> compress{
                current_position_, state.ratio, kHeadDimension, flags};
            dispatch(state.main_compress, kernels_.pipelines().long_compress_decode,
                     compress.data(), 1u);
            if (state.ratio == 4u) {
                const std::array<uint32_t, 4> index_compress{
                    current_position_, 4u, 128u, 3u};
                dispatch(state.index_compress,
                         kernels_.pipelines().long_compress_decode,
                         index_compress.data(), 1u);
                compute_barrier(command);
                if (index_i8_) {
                    const std::array<uint32_t, 4> expand_cache{
                        current_position_ / 4u, 1u, current_position_, 1u};
                    dispatch(state.index_expand,
                             kernels_.pipelines().long_fp4_expand,
                             expand_cache.data(), 1u);
                }
                const std::array<uint32_t, 4> transform{
                    current_position_, kHeads, 0u, 0u};
                dispatch(sets.long_index_query_transform,
                         kernels_.pipelines().long_index_query,
                         transform.data(), kHeads);
                if (index_i8_) {
                    compute_barrier(command);
                    const std::array<uint32_t, 4> expand_query{0u, kHeads, 0u, 0u};
                    dispatch(sets.long_index_query_expand,
                             kernels_.pipelines().long_fp4_expand,
                             expand_query.data(), kHeads);
                }
            }
            compute_barrier(command);
        }
        ...
        if (use_long_attention) {
            LongLayerState& state = long_layers_[layer];
            const uint32_t window_count = kWindow;
            uint32_t compressed_count = 0u;
            if (state.ratio == 4u) {
                const uint32_t available = std::min<uint32_t>(
                    (current_position_ + 1u) / 4u, state.compressed_capacity);
                compressed_count = std::min<uint32_t>(512u, available);
                const bool use_fast = long_fast_ &&
                    (!long_auto_ || current_position_ >= long_auto_crossover_);
                if (!use_fast) {
                    const std::array<uint32_t, 4> score{
                        available, 128u, kHeads, 0u};
                    dispatch(index_i8_ ? state.index_score_i8 : state.index_score,
                             index_i8_ ? kernels_.pipelines().long_index_score_i8 :
                                         kernels_.pipelines().long_index_score,
                             score.data(), divide_up(available, index_score_rows_));
                    compute_barrier(command);
                    const std::array<uint32_t, 4> topk{
                        available, compressed_count, window_count, window_count};
                    dispatch(state.index_topk, kernels_.pipelines().long_topk,
                             topk.data(), 1u);
                    compute_barrier(command);
                } else {
                    if (long_fast_audit_) {
                        const std::array<uint32_t, 4> exact_score{
                            available, 128u, kHeads, 0u};
                        dispatch(index_i8_ ? state.index_score_i8 : state.index_score,
                            index_i8_ ? kernels_.pipelines().long_index_score_i8 :
                                        kernels_.pipelines().long_index_score,
                            exact_score.data(), divide_up(available, index_score_rows_));
                        compute_barrier(command);
                        const std::array<uint32_t, 4> exact_topk{
                            available, compressed_count, 0u, window_count};
                        dispatch(state.exact_topk, kernels_.pipelines().long_topk,
                                 exact_topk.data(), 1u);
                        compute_barrier(command);
                    }
                    const uint32_t blocks = divide_up(available, long_fast_block_);
                    const uint32_t selected_blocks = std::min<uint32_t>(blocks,
                        divide_up(long_fast_candidates_, long_fast_block_));
                    const uint32_t candidate_count = selected_blocks * long_fast_block_;
                    const std::array<uint32_t, 4> coarse{
                        blocks, long_fast_block_, long_fast_representatives_, available};
                    dispatch(state.coarse_score, kernels_.pipelines().long_index_coarse,
                             coarse.data(), divide_up(blocks, 8u));
                    compute_barrier(command);
                    const std::array<uint32_t, 4> coarse_topk{
                        blocks, selected_blocks, 0u, 0u};
                    dispatch(state.coarse_topk, kernels_.pipelines().long_topk,
                             coarse_topk.data(), 1u);
                    compute_barrier(command);
                    const std::array<uint32_t, 4> candidates{
                        candidate_count, long_fast_block_, selected_blocks, available};
                    dispatch(state.candidate_score,
                             kernels_.pipelines().long_index_candidates,
                             candidates.data(), divide_up(candidate_count, 8u));
                    compute_barrier(command);
                    const std::array<uint32_t, 4> candidate_topk{
                        candidate_count, compressed_count, window_count, window_count};
                    dispatch(state.candidate_topk,
                             kernels_.pipelines().long_candidate_topk,
                             candidate_topk.data(), 1u);
                    compute_barrier(command);
                    if (long_fast_audit_) {
                        const std::array<uint32_t, 4> recall{
                            compressed_count, 0u, window_count, 0u};
                        dispatch(state.recall, kernels_.pipelines().long_recall,
                                 recall.data(), 1u);
                        compute_barrier(command);
                    }
                }
            } else if (state.ratio) {
                compressed_count = std::min<uint32_t>(
                    (current_position_ + 1u) / state.ratio,
                    state.compressed_capacity);
                auto* values = static_cast<uint32_t*>(indices_.mapped);
                for (uint32_t i = 0; i < compressed_count; ++i)
                    values[window_count + i] = window_count + i;
                flush_buffer_range(runtime_, indices_,
                    window_count * sizeof(uint32_t),
                    compressed_count * sizeof(uint32_t));
            }
            const std::array<uint32_t, 4> attention{
                kHeads, window_count + compressed_count, window_count, 0u};
            dispatch(state.long_attention,
                     kernels_.pipelines().long_sparse_attention,
                     attention.data(), divide_up(kHeads, 8u));
        } else {
            const uint32_t compressed = sets.ratio4 ?
                (current_position_ + 1u) / 4u : 0u;
        ...
        compute_barrier(command);
        if (use_long_attention) {
            const std::array<uint32_t, 4> inverse{
                current_position_, kHeads,
                index_.compression_ratio(layer) ? 1u : 0u, 0u};
            dispatch(sets.long_inverse_rope,
                     kernels_.pipelines().long_inverse_rope,
                     inverse.data(), kHeads);
        } else {
    ...
    LongFastMetrics long_fast_metrics() {
        LongFastMetrics result{0, 0, long_fast_block_, long_fast_candidates_,
                               long_fast_representatives_, long_fast_,
                               long_fast_audit_};
        if (long_fast_audit_ && long_recall_metrics_.handle) {
            invalidate_buffer(runtime_, long_recall_metrics_);
            const auto* values = static_cast<const uint32_t*>(long_recall_metrics_.mapped);
            result.recalled = values[0]; result.total = values[1];
        }
        return result;
    }

private:
    struct LongLayerState {
        uint32_t ratio = 0;
        uint32_t compressed_capacity = 0;
        Buffer main_cache{};
        Buffer index_cache{};
        Buffer index_cache_i8{};
        VkDeviceSize main_state_offset = 0;
        VkDeviceSize main_state_bytes = 0;
        VkDeviceSize index_state_offset = 0;
        VkDeviceSize index_state_bytes = 0;
        VkDescriptorSet main_compress = VK_NULL_HANDLE;
        VkDescriptorSet index_compress = VK_NULL_HANDLE;
        VkDescriptorSet index_score = VK_NULL_HANDLE;
        VkDescriptorSet index_expand = VK_NULL_HANDLE;
        VkDescriptorSet index_score_i8 = VK_NULL_HANDLE;
        VkDescriptorSet index_topk = VK_NULL_HANDLE;
        VkDescriptorSet coarse_score = VK_NULL_HANDLE;
        VkDescriptorSet coarse_topk = VK_NULL_HANDLE;
        VkDescriptorSet candidate_score = VK_NULL_HANDLE;
        VkDescriptorSet candidate_topk = VK_NULL_HANDLE;
        VkDescriptorSet exact_topk = VK_NULL_HANDLE;
        VkDescriptorSet recall = VK_NULL_HANDLE;
        VkDescriptorSet long_attention = VK_NULL_HANDLE;
    };

    void prepare_stress_history() {
        if (stress_history_prepared_ || !long_context_enabled_ ||
            !std::getenv("DSV4_LONG_CONTEXT_STRESS") ||
            current_position_ <= contiguous_context_tokens_) return;
        const auto started = std::chrono::steady_clock::now();
        for (LongLayerState& state : long_layers_) {
            if (!state.ratio) continue;
            const uint64_t source_records = contiguous_context_tokens_ / state.ratio;
            if (!source_records) continue;
            uint64_t pattern_records = source_records;
            if (const char* tail = std::getenv("DSV4_LONG_STRESS_TAIL_TOKENS"))
                pattern_records = std::clamp<uint64_t>(
                    std::stoull(tail) / state.ratio, 1u, source_records);
            const uint64_t pattern_start = source_records - pattern_records;
            const uint64_t target_records = std::min<uint64_t>(
                current_position_ / state.ratio, state.compressed_capacity);
            if (target_records <= source_records) continue;
            auto repeat_prefix = [&](Buffer& cache, uint64_t record_bytes) {
                auto* base = static_cast<uint8_t*>(cache.mapped);
                uint64_t filled = source_records;
                while (filled < target_records) {
                    const uint64_t copy_records = std::min(
                        pattern_records, target_records - filled);
                    std::memcpy(base + filled * record_bytes,
                        base + pattern_start * record_bytes,
                        static_cast<size_t>(copy_records * record_bytes));
                    filled += copy_records;
                }
                flush_buffer_range(runtime_, cache, 0,
                    target_records * record_bytes);
            };
            repeat_prefix(state.main_cache,
                          kHeadDimension * sizeof(uint16_t));
            if (state.ratio == 4u)
                repeat_prefix(state.index_cache, 17u * sizeof(uint32_t));
            if (state.ratio == 4u && index_i8_)
                repeat_prefix(state.index_cache_i8, 33u * sizeof(uint32_t));
        }
        stress_history_prepared_ = true;
        stress_history_fill_seconds_ = std::chrono::duration<double>(
            std::chrono::steady_clock::now() - started).count();
        std::cout << "DeepSeek stress history: repeated "
                  << contiguous_context_tokens_
                  << " real prompt tokens across populated context in "
                  << stress_history_fill_seconds_ << " s\n";
    }
    ...
    struct LayerSets {
        ...
        VkDescriptorSet long_main_wkv = VK_NULL_HANDLE;
        VkDescriptorSet long_main_gate = VK_NULL_HANDLE;
        VkDescriptorSet long_index_wkv = VK_NULL_HANDLE;
        VkDescriptorSet long_index_gate = VK_NULL_HANDLE;
        VkDescriptorSet long_index_wq = VK_NULL_HANDLE;
        VkDescriptorSet long_index_weights = VK_NULL_HANDLE;
        VkDescriptorSet long_query_rope = VK_NULL_HANDLE;
        VkDescriptorSet long_kv_rope = VK_NULL_HANDLE;
        VkDescriptorSet long_index_query_transform = VK_NULL_HANDLE;
        VkDescriptorSet long_index_query_expand = VK_NULL_HANDLE;
        VkDescriptorSet long_inverse_rope = VK_NULL_HANDLE;
    ...
    void initialize_long_context_state() {
        if (!std::getenv("DSV4_LONG_CONTEXT_STRESS") &&
            !std::getenv("DSV4_LONG_CONTEXT_REAL")) return;
        const char* budget_text = std::getenv("DSV4_CONTEXT_GIB");
        if (!budget_text)
            throw std::runtime_error(
                "DeepSeek long context requires DSV4_CONTEXT_GIB");
        const double gib = std::stod(budget_text);
        if (gib < 0.05 || gib > 16.0)
            throw std::runtime_error("DSV4_CONTEXT_GIB must be 0.05..16");
        long_context_bytes_ = static_cast<uint64_t>(
            gib * 1024.0 * 1024.0 * 1024.0);
        // Find the exact capacity under the selected context budget. Main KV is
        // BF16; ratio-4 index keys use the lossless packed representation of
        // their already FP4-simulated values. Include persistent compressor
        // state rather than relying on a coarse bytes/token estimate.
        const auto storage_bytes = [&](uint64_t tokens) {
            uint64_t total = 0;
            for (uint32_t layer = 0; layer < kLayers; ++layer) {
                const uint32_t ratio = index_.compression_ratio(layer);
                if (!ratio) continue;
                const uint64_t capacity = tokens / ratio + 1u;
                total += capacity * kHeadDimension * sizeof(uint16_t);
                const uint32_t coff = ratio == 4u ? 2u : 1u;
                total += uint64_t(coff) * ratio * coff * kHeadDimension *
                         2u * sizeof(float);
                if (ratio == 4u) {
                    total += capacity * 17u * sizeof(uint32_t);
                    if (index_i8_)
                        total += capacity * 33u * sizeof(uint32_t);
                    total += 2ull * 4ull * 256ull * 2ull * sizeof(float);
                }
            }
            return total;
        };
        uint64_t low = kWindow, high = kMaximumContext;
        while (low < high) {
            const uint64_t middle = low + (high - low + 1u) / 2u;
            if (storage_bytes(middle) <= long_context_bytes_) low = middle;
            else high = middle - 1u;
        }
        if (const char* exact_text = std::getenv("DSV4_CONTEXT_TOKENS")) {
            const uint64_t requested = std::stoull(exact_text);
            if (requested < kWindow || requested > kMaximumContext)
                throw std::runtime_error(
                    "DSV4_CONTEXT_TOKENS must be 128..1048576");
            if (storage_bytes(requested) > long_context_bytes_)
                throw std::runtime_error(
                    "DSV4_CONTEXT_TOKENS exceeds DSV4_CONTEXT_GIB budget");
            low = requested;
        }
        long_context_tokens_ = low;
        long_context_allocated_bytes_ = storage_bytes(long_context_tokens_);
        if (long_context_tokens_ < kWindow)
            throw std::runtime_error("DeepSeek long-context allocation is too small");
        long_context_enabled_ = true;
        long_layers_.resize(kLayers);
        uint64_t main_state_floats = 0, index_state_floats = 0;
        for (uint32_t layer = 0; layer < kLayers; ++layer) {
            LongLayerState& state = long_layers_[layer];
            state.ratio = index_.compression_ratio(layer);
            if (!state.ratio) continue;
            state.compressed_capacity = static_cast<uint32_t>(
                long_context_tokens_ / state.ratio + 1u);
            const VkDeviceSize main_bytes = static_cast<VkDeviceSize>(
                state.compressed_capacity) * kHeadDimension * sizeof(uint16_t);
            state.main_cache = create_host_buffer_uninitialized(runtime_, main_bytes);
            std::memset(state.main_cache.mapped, 0, static_cast<size_t>(main_bytes));
            flush_buffer_range(runtime_, state.main_cache, 0, main_bytes);
            const uint32_t coff = state.ratio == 4u ? 2u : 1u;
            state.main_state_offset = main_state_floats * sizeof(float);
            state.main_state_bytes = static_cast<VkDeviceSize>(
                coff * state.ratio * coff * kHeadDimension * 2u) * sizeof(float);
            main_state_floats += state.main_state_bytes / sizeof(float);
            if (state.ratio == 4u) {
                // The official index path FP4-simulates each 128-wide key.
                // Preserve those exact values in packed form: 16 nibble words
                // plus one word containing four power-of-two scale exponents.
                const VkDeviceSize index_bytes = static_cast<VkDeviceSize>(
                    state.compressed_capacity) * 17u * sizeof(uint32_t);
                state.index_cache = create_host_buffer_uninitialized(runtime_, index_bytes);
                std::memset(state.index_cache.mapped, 0, static_cast<size_t>(index_bytes));
                flush_buffer_range(runtime_, state.index_cache, 0, index_bytes);
                if (index_i8_) {
                    const VkDeviceSize expanded_bytes = static_cast<VkDeviceSize>(
                        state.compressed_capacity) * 33u * sizeof(uint32_t);
                    state.index_cache_i8 = create_host_buffer_uninitialized(
                        runtime_, expanded_bytes);
                    std::memset(state.index_cache_i8.mapped, 0,
                                static_cast<size_t>(expanded_bytes));
                    flush_buffer_range(runtime_, state.index_cache_i8, 0,
                                       expanded_bytes);
                }
                state.index_state_offset = index_state_floats * sizeof(float);
                state.index_state_bytes = 2ull * 4ull * 256ull * 2ull * sizeof(float);
                index_state_floats += state.index_state_bytes / sizeof(float);
            }
        }
        long_main_state_ = create_buffer(runtime_,
            std::max<uint64_t>(main_state_floats, 1u) * sizeof(float));
        long_index_state_ = create_buffer(runtime_,
            std::max<uint64_t>(index_state_floats, 1u) * sizeof(float));
        auto initialize_state = [](Buffer& buffer) {
            auto* values = static_cast<float*>(buffer.mapped);
            const size_t count = static_cast<size_t>(buffer.size / sizeof(float));
            std::fill(values, values + count, 0.0f);
        };
        initialize_state(long_main_state_);
        initialize_state(long_index_state_);
        for (const LongLayerState& state : long_layers_) {
            if (state.main_state_bytes) {
                auto* base = reinterpret_cast<float*>(
                    static_cast<uint8_t*>(long_main_state_.mapped) +
                    state.main_state_offset);
                const size_t values = static_cast<size_t>(
                    state.main_state_bytes / sizeof(float));
                std::fill(base + values / 2u, base + values,
                          -std::numeric_limits<float>::infinity());
            }
            if (state.index_state_bytes) {
                auto* base = reinterpret_cast<float*>(
                    static_cast<uint8_t*>(long_index_state_.mapped) +
                    state.index_state_offset);
                const size_t values = static_cast<size_t>(
                    state.index_state_bytes / sizeof(float));
                std::fill(base + values / 2u, base + values,
                          -std::numeric_limits<float>::infinity());
            }
        }
        flush_buffer(runtime_, long_main_state_);
        flush_buffer(runtime_, long_index_state_);
        long_kv_projection_ = create_device_buffer(runtime_, 1024u * sizeof(float));
        long_score_projection_ = create_device_buffer(runtime_, 1024u * sizeof(float));
        long_index_kv_projection_ = create_device_buffer(runtime_, 256u * sizeof(float));
        long_index_score_projection_ = create_device_buffer(runtime_, 256u * sizeof(float));
        long_index_query_ = create_device_buffer(runtime_, 8192u * sizeof(float));
        long_index_query_packed_ = create_device_buffer(runtime_,
            64u * 17u * sizeof(uint32_t));
        if (index_i8_)
            long_index_query_i8_ = create_device_buffer(runtime_,
                64u * 33u * sizeof(uint32_t));
        long_index_weights_ = create_device_buffer(runtime_, 64u * sizeof(float));
        long_index_scores_ = create_device_buffer(runtime_,
            static_cast<VkDeviceSize>(long_context_tokens_ / 4u + 1u) * sizeof(float));
        if (long_fast_) {
            const uint64_t maximum_keys = long_context_tokens_ / 4u + 1u;
            const uint64_t maximum_blocks =
                (maximum_keys + long_fast_block_ - 1u) / long_fast_block_;
            long_coarse_scores_ = create_device_buffer(runtime_,
                maximum_blocks * sizeof(float));
            long_block_indices_ = create_device_buffer(runtime_,
                maximum_blocks * sizeof(uint32_t));
            long_candidate_scores_ = create_device_buffer(runtime_,
                static_cast<uint64_t>(long_fast_candidates_) * sizeof(float));
            long_candidate_positions_ = create_device_buffer(runtime_,
                static_cast<uint64_t>(long_fast_candidates_) * sizeof(uint32_t));
            long_exact_indices_ = create_device_buffer(runtime_, 512u * sizeof(uint32_t));
            long_recall_metrics_ = create_buffer(runtime_, 4u * sizeof(uint32_t));
            std::memset(long_recall_metrics_.mapped, 0, 4u * sizeof(uint32_t));
            flush_buffer(runtime_, long_recall_metrics_);
            std::cout << "DeepSeek fast retrieval: block " << long_fast_block_
                      << ", candidates " << long_fast_candidates_
                      << ", representatives " << long_fast_representatives_
                      << (long_fast_audit_ ? " (full-scan recall audit)\n" : "\n");
        }
        std::cout << "DeepSeek learned long-context capacity: "
                  << long_context_tokens_ << " tokens, BF16 main / packed-FP4 index host state "
                  << double(long_context_allocated_bytes_) / double(1ull << 30)
                  << " GiB (official maximum " << kMaximumContext << ")\n";
        _putenv_s("DSV4_CONTEXT_CAPACITY",
                  std::to_string(long_context_tokens_).c_str());
    }
    ...
    void build_layer_sets() {
        ...
        for (uint32_t layer = 0; layer < kLayers; ++layer) {
            ...
            if (long_context_enabled_) {
                LongLayerState& state = long_layers_[layer];
                sets.long_query_rope = make_set({whole(executor_.query()), whole(ones_),
                    whole(executor_.query())});
                sets.long_kv_rope = make_set({whole(executor_.kv()),
                    data(tensor("attn.kv_norm.weight")), whole(executor_.kv())});
                sets.long_inverse_rope = make_set({whole(executor_.context()),
                    whole(executor_.query())});
                const VkDeviceSize window_layer_bytes =
                    static_cast<VkDeviceSize>(kWindow + 32u) *
                    kHeadDimension * sizeof(float);
                const DescriptorRange compressed = state.ratio ?
                    whole(state.main_cache) : kernels_.dummy();
                state.long_attention = make_set({whole(executor_.query()),
                    arena_range(executor_.kv_cache(),
                        static_cast<VkDeviceSize>(layer) * window_layer_bytes,
                        kWindow * kHeadDimension * sizeof(float)),
                    compressed, whole(indices_), data(tensor("attn.attn_sink")),
                    whole(executor_.context())});
                if (state.ratio) {
                    const uint32_t projection_rows = state.ratio == 4u ? 1024u : 512u;
                    const TensorView& main_wkv = q8("attn.compressor.wkv.weight",
                                                    projection_rows, kDimension);
                    const TensorView& main_gate = q8("attn.compressor.wgate.weight",
                                                     projection_rows, kDimension);
                    sets.long_main_wkv = q8_set(whole(quantized_), main_wkv, arena,
                        base, arena_range(long_kv_projection_, 0,
                            projection_rows * sizeof(float)));
                    sets.long_main_gate = q8_set(whole(quantized_), main_gate, arena,
                        base, arena_range(long_score_projection_, 0,
                            projection_rows * sizeof(float)));
                    state.main_compress = make_set({whole(long_kv_projection_),
                        whole(long_score_projection_),
                        data(tensor("attn.compressor.ape")),
                        data(tensor("attn.compressor.norm.weight")),
                        arena_range(long_main_state_, state.main_state_offset,
                                    state.main_state_bytes),
                        whole(state.main_cache)});
                }
                if (state.ratio == 4u) {
                    const TensorView& index_wkv = q8(
                        "attn.indexer.compressor.wkv.weight", 256u, kDimension);
                    const TensorView& index_gate = q8(
                        "attn.indexer.compressor.wgate.weight", 256u, kDimension);
                    sets.long_index_wkv = q8_set(whole(quantized_), index_wkv,
                        arena, base, whole(long_index_kv_projection_));
                    sets.long_index_gate = q8_set(whole(quantized_), index_gate,
                        arena, base, whole(long_index_score_projection_));
                    const TensorView& index_wq = q8("attn.indexer.wq_b.weight",
                                                    8192u, 1024u);
                    sets.long_index_wq = q8_set(whole(quantized_), index_wq,
                        arena, base, whole(long_index_query_));
                    const TensorView& index_weights = q8(
                        "attn.indexer.weights_proj.weight", 64u, kDimension);
                    sets.long_index_weights = q8_set(whole(quantized_), index_weights,
                        arena, base, whole(long_index_weights_));
                    sets.long_index_query_transform = make_set({
                        whole(long_index_query_), whole(long_index_query_packed_)});
                    if (index_i8_)
                        sets.long_index_query_expand = make_set({
                            whole(long_index_query_packed_),
                            whole(long_index_query_i8_)});
                    state.index_compress = make_set({whole(long_index_kv_projection_),
                        whole(long_index_score_projection_),
                        data(tensor("attn.indexer.compressor.ape")),
                        data(tensor("attn.indexer.compressor.norm.weight")),
                        arena_range(long_index_state_, state.index_state_offset,
                                    state.index_state_bytes),
                        whole(state.index_cache)});
                    state.index_score = make_set({whole(long_index_query_packed_),
                        whole(state.index_cache), whole(long_index_weights_),
                        whole(long_index_scores_)});
                    if (index_i8_) {
                        state.index_expand = make_set({whole(state.index_cache),
                                                      whole(state.index_cache_i8)});
                        state.index_score_i8 = make_set({whole(long_index_query_i8_),
                            whole(state.index_cache_i8), whole(long_index_weights_),
                            whole(long_index_scores_)});
                    }
                    state.index_topk = make_set({whole(long_index_scores_),
                                                 whole(indices_)});
                    if (long_fast_) {
                        state.coarse_score = make_set({whole(long_index_query_packed_),
                            whole(state.index_cache), whole(long_index_weights_),
                            whole(long_coarse_scores_)});
                        state.coarse_topk = make_set({whole(long_coarse_scores_),
                                                     whole(long_block_indices_)});
                        state.candidate_score = make_set({whole(long_index_query_packed_),
                            whole(state.index_cache), whole(long_index_weights_),
                            whole(long_block_indices_), whole(long_candidate_scores_),
                            whole(long_candidate_positions_)});
                        state.candidate_topk = make_set({whole(long_candidate_scores_),
                            whole(long_candidate_positions_), whole(indices_)});
                        state.exact_topk = make_set({whole(long_index_scores_),
                                                    whole(long_exact_indices_)});
                        state.recall = make_set({whole(long_exact_indices_),
                            whole(indices_), whole(long_recall_metrics_)});
                    }
                }
            }
    ...
    void update_indices(uint32_t position) {
        ...
        if (position < kWindow) {
            ...
            const uint32_t oldest = (position + 1u) % kWindow;
            for (uint32_t index = 0; index < kWindow; ++index)
                values[count++] = (oldest + index) % kWindow;
        }
        while (count < 640u) values[count++] = UINT32_MAX;
    ...
    Buffer long_main_state_{};
    Buffer long_index_state_{};
    Buffer long_kv_projection_{};
    Buffer long_score_projection_{};
    Buffer long_index_kv_projection_{};
    Buffer long_index_score_projection_{};
    Buffer long_index_query_{};
    Buffer long_index_query_packed_{};
    Buffer long_index_query_i8_{};
    Buffer long_index_weights_{};
    Buffer long_index_scores_{};
    Buffer long_coarse_scores_{};
    Buffer long_block_indices_{};
    Buffer long_candidate_scores_{};
    Buffer long_candidate_positions_{};
    Buffer long_exact_indices_{};
    Buffer long_recall_metrics_{};
    std::vector<LongLayerState> long_layers_;
    std::vector<Buffer> long_context_buffers_;
    Buffer long_context_checksum_{};
    std::vector<VkDescriptorSet> long_context_sets_;
    std::vector<uint32_t> long_context_scan_words_;
    uint64_t long_context_bytes_ = 0;
    uint64_t long_context_allocated_bytes_ = 0;
    uint64_t long_context_tokens_ = 0;
    bool long_context_enabled_ = false;
    CommonSets common_{};
    GlobalSets global_{};
    ...
    uint32_t current_position_ = 0;
    uint32_t contiguous_context_tokens_ = 0;
    bool stress_history_prepared_ = false;
    double stress_history_fill_seconds_ = 0.0;
    ...
    uint32_t index_score_rows_ = 8u;
    bool long_fast_ = false;
    bool index_i8_ = false;
    bool long_auto_ = false;
    // The clean populated-context matrix brackets the crossover between 64K
    // (effectively tied) and 128K (1.106/1.270 tok/s Exact/Fast).  Use the
    // midpoint of that measured interval so Auto changes on active history,
    // not only at the final slot of a 128K allocation.
    uint32_t long_auto_crossover_ = 98304u;
    bool long_fast_audit_ = false;
    // Real-prompt history audit selected this quality-oriented point.  The
    // earlier 128/16384 layout was faster but recovered only ~67% of Exact's
    // final Top-512 positions at 1M; 32/4096 recovered 100% on the fixed
    // repeated-real-prompt matrix while retaining a 2.07x 1M speedup.
    uint32_t long_fast_block_ = 32u;
    uint32_t long_fast_candidates_ = 4096u;
    uint32_t long_fast_representatives_ = 4u;
...
int dsv4_cli_main(int argc, char** argv) {
    ...
    try {
        ...
        const std::filesystem::path q4_long = directory / "model-q4g64-long.ovs";
        const bool request_long_context =
            std::getenv("DSV4_LONG_CONTEXT_STRESS") != nullptr ||
            std::getenv("DSV4_LONG_CONTEXT_REAL") != nullptr;
        if (request_long_context && !std::filesystem::exists(q4_long))
            throw std::runtime_error(
                "DeepSeek learned long-context container model-q4g64-long.ovs is absent");
        ...
        const char* shared_name = request_long_context ? "model-q4g64-long.ovs" :
            (use_q4_global ? "model-q4g64-global.ovs" :
        ...
        dsv4::DeepSeekProgram::LongFastMetrics long_fast_metrics{};
        ...
        const auto started = std::chrono::steady_clock::now();
        {
            ...
            long_fast_metrics = program.long_fast_metrics();
        ...
        std::cout << "\nelapsed: " << seconds << " s\n"
                  ...
                  << " GiB\n";
        if (long_fast_metrics.enabled) {
            const double recall = long_fast_metrics.total ?
                100.0 * double(long_fast_metrics.recalled) /
                    double(long_fast_metrics.total) : 0.0;
            std::cout << "fast long retrieval block/candidates/representatives: "
                      << long_fast_metrics.block << '/'
                      << long_fast_metrics.candidates << '/'
                      << long_fast_metrics.representatives << "\n"
                      << "fast long Top-512 recall: "
                      << long_fast_metrics.recalled << '/'
                      << long_fast_metrics.total << " (" << recall << "%)"
                      << (long_fast_metrics.audited ? "\n" : " (not audited in performance mode)\n");
        }

...
public:
    DeviceExpertCache(const Runtime& runtime, uint32_t slots)
        ...
        for (uint32_t index = 0; index < kLayers; ++index) {
            ...
            layer.coactivation.assign(kExperts * kExperts, 0);
    ...
    Selection resolve(uint32_t layer_index,
                      ...
                      float approximate_threshold) {
        Layer& layer = layers_.at(layer_index);
        const uint32_t capacity = capacities_.at(layer_index);
        Selection result{};
        std::vector<bool> reserved(capacity);
        for (uint32_t left : experts) for (uint32_t right : experts) {
            uint16_t& value = layer.coactivation[left * kExperts + right];
        ...
        // Isolated approximate experiment: a sufficiently low-weight missing
        // expert may reuse a co-activated selected expert that is already
        // resident.  Authoritative routing weights and all exact paths remain
        // untouched; this branch is disabled at threshold zero.
        if (approximate_threshold > 0.0f) {
            for (uint32_t rank = 0; rank < kTopK; ++rank) {
                if (result.slots[rank] != UINT32_MAX ||
                    weights[rank] > approximate_threshold)
                    continue;
                uint32_t proxy_rank = UINT32_MAX;
                uint16_t best = 0;
                for (uint32_t other = 0; other < kTopK; ++other) {
                    if (result.slots[other] == UINT32_MAX) continue;
                    const uint16_t score = layer.coactivation[
                        experts[rank] * kExperts + experts[other]];
                    if (proxy_rank == UINT32_MAX || score > best) {
                        proxy_rank = other;
                        best = score;
                    }
                }
                if (proxy_rank != UINT32_MAX) {
                    result.slots[rank] = result.slots[proxy_rank];
                    ++substitutions_;
                }
            }
        }
    ...
    uint64_t device_bytes() const { return device_bytes_; }
    void reset_metrics() { hits_ = misses_ = substitutions_ = 0; }
...
private:
    ...
    struct Layer {
        ...
        std::vector<uint16_t> coactivation;
    };
    const Runtime& runtime_;
    uint32_t slots_;
    ...
    uint64_t clock_ = 0, hits_ = 0, misses_ = 0, substitutions_ = 0;
    uint64_t device_bytes_ = 0;
};
...
public:
    QwenEngine(const Runtime& runtime, const SharedIndex& index,
        ...
        if (const char* text = std::getenv("QWEN_BUDDY_THRESHOLD"))
            buddy_threshold_ = std::stof(text);
        if (buddy_threshold_ < 0.0f || buddy_threshold_ > 0.25f)
            throw std::runtime_error("QWEN_BUDDY_THRESHOLD must be 0..0.25");
    ...
    uint64_t buddy_substitutions() const {
        return device_cache_.substitutions();
    }
        ...
        *static_cast<uint32_t*>(token_.mapped) = token;
        ...
        for (uint32_t layer = 0; layer < kLayers; ++layer) {
            ...
            const DeviceExpertCache::Selection selection =
                device_cache_.resolve(layer, experts, route_weights,
                                      buddy_threshold_);
...
int qwen35_cli_main(int argc, char** argv) {
    ...
    try {
        ...
        uint64_t device_hits = 0, device_misses = 0, buddy_substitutions = 0;
        ...
        uint32_t host_slots = 0;
        {
            ...
            buddy_substitutions = engine.buddy_substitutions();
        ...
        std::cout << "\ndecode throughput: "
                  ...
                  << ram_bypasses << '\n'
                  << "approximate BuddyMoE substitutions: "
                  << buddy_substitutions << '\n'

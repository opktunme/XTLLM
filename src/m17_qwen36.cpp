...
// Qwen3.6-35B-A3B text-only executor.  This is deliberately separate from
...
constexpr uint32_t kFullLayers = 10;
constexpr uint32_t kLinearLayers = 30;
...
public:
    explicit SharedIndex(const std::filesystem::path& path) : path_(path) {
        ...
        if (std::memcmp(header_.magic, "OQ36SHR\0", 8) != 0 ||
            ...
            throw std::runtime_error("Unsupported Qwen3.6 shared container");
...
public:
    ...
    explicit ExpertFile(const std::filesystem::path& path) : path_(path) {
        ...
        if (std::memcmp(header_.magic, "OQ36EXP\0", 8) != 0 ||
            header_.version != 1 || header_.header_bytes != kHeaderBytes ||
            header_.dimension != kDim || header_.moe_dimension != kMoeDim ||
...
public:
    ...
    HostExpertCache(ExpertFile& file, uint64_t budget_bytes) : file_(file) {
        tiny_lfu_ = std::getenv("QWEN36_HOST_TINYLFU") != nullptr;
...
public:
    Kernels(const Runtime& runtime, const std::filesystem::path& directory)
        ...
        pipelines_.expert_gate = load("qwen36_expert_gate_up_q4");
        pipelines_.expert_down = load("qwen36_expert_down_q4");
        ...
        pipelines_.attention = load("qwen36_long_attention_partial");
        ...
        pipelines_.delta = load("qwen36_delta_recurrent_norm");
...
static uint32_t requested_context_tokens() {
    if (const char* exact = std::getenv("QWEN36_CONTEXT_TOKENS")) {
        const uint64_t value = std::stoull(exact);
        if (value < 128 || value > UINT32_MAX)
            throw std::runtime_error("QWEN36_CONTEXT_TOKENS must be 128..2^32-1");
        return static_cast<uint32_t>(value);
    }
    if (const char* text = std::getenv("QWEN36_CONTEXT_GIB")) {
        const double gib = std::stod(text);
        if (gib < 0.05 || gib > 16.0)
            throw std::runtime_error("QWEN36_CONTEXT_GIB must be 0.05..16");
...
public:
    QwenEngine(const Runtime& runtime, const SharedIndex& index,
        ...
        progressive_experts_ =
            std::getenv("QWEN36_PROGRESSIVE_EXPERTS") != nullptr;
    ...
    std::vector<uint32_t> generate(const Tokenizer& tokenizer,
        ...
        if (uint64_t(prompt.size()) + count > max_context_)
            throw std::runtime_error("Qwen3.6 prompt plus generation exceeds configured context");
#endif
        uint32_t position = 0;
        uint32_t next = 0;
        for (uint32_t token : prompt) next = run_token(token, position++);
        if (std::getenv("QWEN36_FILL_RAM_CACHE")) {
...
#ifdef OVLLM_LONG_CONTEXT_FORK
        if (std::getenv("QWEN36_LONG_CONTEXT_STRESS")) {
            if (max_context_ <= count + 2u)
                throw std::runtime_error("Qwen3.6 context is too small for stress decode");
            const uint32_t target = max_context_ - count;
            expand_context_for_stress(prompt.size() - 1u, target);
            position = target;
            std::cout << "Qwen3.6 long-context stress positions: " << target
                      << ".." << (max_context_ - 1u) << "\n";
        }
#endif
...
private:
    ...
    void allocate_buffers() {
        ...
        expert_quant_ = device(1056ull * 4);
    ...
    void record_attention(VkCommandBuffer command, uint32_t layer,
        ...
        if (full_attention(layer)) {
            push = {8192, kDim, kDim / 4, 0};
            kernels_.dispatch(command, kernels_.p().q4, sets.qgate, &push, 1024);
            ...
            kernels_.dispatch(command, kernels_.p().q4, sets.gdn_z, &push,
                              kLinearValue / 8);
            push = {64, kDim, kDim / 4, 0};
            kernels_.dispatch(command, kernels_.p().q4, sets.ab, &push, 8);
            compute_barrier(command);
...
static uint64_t ram_budget() {
    const char* text = std::getenv("QWEN36_RAM_GIB");
    const double gib = text ? std::stod(text) : 24.0;
    if (gib < 2.0 || gib > 56.0)
        throw std::runtime_error("QWEN36_RAM_GIB must be 2..56");
    return static_cast<uint64_t>(gib * 1024.0 * 1024.0 * 1024.0);
}

static uint32_t device_slots() {
    const char* text = std::getenv("QWEN36_DEVICE_SLOTS_PER_LAYER");
    const uint32_t slots = text ? static_cast<uint32_t>(std::stoul(text)) : 96;
    if (slots < kTopK || slots > 128)
        throw std::runtime_error("QWEN36_DEVICE_SLOTS_PER_LAYER must be 8..128");
    return slots;
}

} // namespace qwen36

int qwen36_cli_main(int argc, char** argv) {
    Runtime runtime{};
    try {
        if (argc < 3) {
            std::cerr << "usage: amd_qwen36.exe <runtime-dir> "
                         "<prompt|--inspect|--tokenize> [new-tokens]\n";
            return 2;
        }
        const std::filesystem::path directory = argv[1];
        dsv4::ReadOnlyMapping tokenizer_file((directory / "tokenizer.ovb").string());
        qwen36::Tokenizer tokenizer(tokenizer_file);
        if (std::strcmp(argv[2], "--tokenize") == 0) {
            if (argc < 4) throw std::runtime_error("tokenize text required");
            const bool thinking = std::getenv("QWEN36_NO_THINK") == nullptr;
        ...
        qwen36::SharedIndex index(directory / "model-q4g64.ovs");
        qwen36::ExpertFile inspect_experts(directory / "experts-q4g64.ovx");
        if (std::strcmp(argv[2], "--inspect") == 0) {
            std::cout << "Qwen3.6-35B-A3B runtime containers validated\n";
            return 0;
        }
        ...
        const uint64_t budget = qwen36::ram_budget();
        const uint32_t slots = qwen36::device_slots();
        const bool thinking = std::getenv("QWEN36_NO_THINK") == nullptr;
        std::vector<uint32_t> result;
        ...
        std::cerr << "Qwen3.6 runtime error: " << error.what() << '\n';
...
#ifndef OVLLM_QWEN36_RUNTIME_ONLY
int main(int argc, char** argv) {
    return qwen36_cli_main(argc, argv);
}
#endif
